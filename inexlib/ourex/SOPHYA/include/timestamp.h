...
public:
 ...
 //! Sets the value of the time stamp from a string in the format YYYY-MM-DDThh:mm:ss 
  inline void Set(std::string const & datim)  { Set(datim.c_str()); }
  ...
  //! Sets the value of the date (format: DD/MM/YYYY) 
  inline void SetDate(std::string const& date) { SetDate(date.c_str()); }
  ...
  //! Sets the value of the time of day (format: hh:mm:ss[.ss]) 
  inline void SetHour(std::string const& hour) { SetHour(hour.c_str()); }
  ...
  std::string ToString(StrFmt fmt=FmtPackedDateTime) const;
  ...
  virtual void    Print(std::ostream& os, StrFmt fmt=FmtDateTime)  const;
...
inline std::ostream& operator << (std::ostream& s, TimeStamp const & ts)
  {  ts.Print(s);  return(s);  }
