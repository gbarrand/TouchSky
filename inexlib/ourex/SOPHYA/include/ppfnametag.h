...
public:
  ...
  PPFNameTag(std::string const & tn) { _tname = tn; }
  ...
  inline std::string const & operator= (std::string const & s)  { _tname = s; return s; } 
  inline operator  std::string() const { return _tname; }
protected:
  std::string _tname; 
};
