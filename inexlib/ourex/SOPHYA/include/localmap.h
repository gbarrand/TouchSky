...
template<class T>
class LocalMap : public SphericalMap<T>
{
...
inline T  operator()(int_4 k) const {return(this->PixVal(k));}
inline T& operator()(int_4 ix, int_4 iy) {return this->PixVal(iy*nSzX_+ix);};
inline T  operator()(int_4 ix, int_4 iy) const {return this->PixVal(iy*nSzX_+ix);};

// Acces a un pixel 
inline T& operator()(double theta,double phi) {return(this->PixValSph(theta,phi));};
inline T  operator()(double theta,double phi) const {return(this->PixValSph(theta,phi));};
...
void SetSize(double angleX,double angleY);
  ...
  */ 
inline virtual std::string TypeOfMap() const {return std::string("LOCAL");};
...
//! for declaring tempory object (for CloneOrShare() )  (see NDataBlock documentation
...
extern template class LocalMap<int_8>;
extern template class LocalMap<r_4>;
extern template class LocalMap<r_8>;
extern template class LocalMap< std::complex<r_4> >;
extern template class LocalMap< std::complex<r_8> >;
#ifdef SO_LDBLE128
extern template class LocalMap<r_16>;
extern template class LocalMap< std::complex<r_16> >;
