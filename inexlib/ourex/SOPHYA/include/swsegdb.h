...
class DataSwapperInterface {
public:
  DataSwapperInterface() : mode_(std::ios::in|std::ios::out) { }
...
public:
  ...
  */
  ...
  SwSegDataBlock(DataSwapperInterface<T> & dsw, std::vector<int_8> const & swpos, size_t segsz, size_t nitem=1)
  {
    mSRef = NULL;
  ...
  */
  ...
  virtual void SetSize(size_t segsz, size_t nbseg=0, size_t nitem=1)
  {
    ...
    /* DEL  Reza 09/03/2018 DataSwapperInterface<T> * dswp=NULL;
       if (mSRef)  dswp = mSRef->swapper->Clone();  le Clone est fait par Delete(true) */
