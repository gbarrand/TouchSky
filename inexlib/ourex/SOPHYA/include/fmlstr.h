...
public:
  ...
  inline uint_2 size() const { return N; } 
  ...
  { os << "FMLString<" << N << "> length=" << length() << " buff:" << buff_ << std::endl; return os; }
  //! print all information about the object on cout
  inline std::ostream&  printall()  const  
...
template <uint_2 N>
std::ostream& operator << (std::ostream& s, FMLString<N> const & fsl)
