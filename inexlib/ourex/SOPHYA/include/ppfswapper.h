...
public:
  ...
  */
  ...
  virtual void  ReadFromSwap(int_8 idx, int_8 swp, T* d, size_t sz)
  {
    ...
    int_8 cspos=-1; 
    if (!mSRef->ppfis->getStream().isSeekable())  
      throw NotAvailableOperation("PPFDataSwapper<T>::ReadFromSwap() - No PInPersist stream  NOT Seekable !");
    cspos=mSRef->ppfis->getStream().tellg();
    if (mSRef->ppfis->GotoPositionTag(swp)) {
      PPF_TPointer_IO<T>::Read(*(mSRef->ppfis), d, sz);
      if (mSRef->ppfis->getStream().isSeekable()) mSRef->ppfis->getStream().seekg(cspos);
    }
    // Cela ne devrait pas etre necessaire - la methode GotoPositionTag() envoie elle meme une exception 
    else throw FileFormatExc("PPFDataSwapper<T>::ReadFromSwap() - Error calling GotoPositionTag !"); 
    return;
  }
...
protected:
  ...
  void Init(POutPersist* os, bool ados, PInPersist* is, bool adis)
  {
    ...
    if (os) DataSwapperInterface<T>::mode_ |= std::ios::out; 
    if (is) DataSwapperInterface<T>::mode_ |= std::ios::in; 
    return;
  }
  //! Initializes the shared data structure
