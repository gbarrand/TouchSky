//-----------------------------------------------------------
// Classes DataTableRow , DTCell , DataTableRowPtr ...
//    Classes pour representer les contenus des cellules et 
//    les lignes des DataTables 
...
//! Constructor - For use by BaseDataTable class
  DataTableRow( std::vector<std::string> const & colnames, std::vector<size_t> const & colvsz);
//! Copy constructor
...
//! Return a reference to the value of column \b colname. (or the first element of)
  inline MuTyV&     operator()  (std::string const colname, size_t iv=0)   { return getPtr(colname)[iv]; }
//! Return the MuTyV pointer for column number \b k. 
  MuTyV*     getPtr(size_t k);
//! Return the MuTyV pointer for column named \b colname. 
  MuTyV*     getPtr(std::string const& colname);
...
protected:
  MuTyV * mtv_;
  size_t size_;
  std::vector< size_t > colseqp_;    // column index to sequential position in mtv_
  std::map<std::string , size_t> nm2idx_;    // column name to column index 
};

/*! Prints the DataTableRow object content on the output stream ( */
  inline std::ostream& operator << (std::ostream& s, DataTableRow const & row)
...
enum DataTableCellType { DTC_IntegerType, DTC_LongType, 
			 ...
			 DTC_DateTimeType, DTC_StringType, DTC_CharacterType,
			 DTC_FMLStr16Type, DTC_FMLStr64Type,
			 DTC_FlagVec16Type, DTC_FlagVec64Type};
...
public:
  ...
  //! Assignment operators from char value
  inline int_4 operator= (char v)
  { 
    if (typ_==DTC_CharacterType) *(u_ptr_.cp)=v;
    else if (typ_==DTC_IntegerType) *(u_ptr_.i4)=(int_4)v;
  ...
  inline std::complex<r_4> operator= (std::complex<r_4> const& v)
  {
    if (typ_==DTC_ComplexType) *(u_ptr_.z4)=v;
    else if (typ_==DTC_DoubleComplexType) *(u_ptr_.z8)=std::complex<r_8>((r_8)v.real(), (r_8)v.imag());
    return v;
  }
  //! Assignment operators from double precision complex ( complex<r_8> )
  inline std::complex<r_8> operator= (std::complex<r_8> const& v)
  {
    if (typ_==DTC_ComplexType) *(u_ptr_.z4)=std::complex<r_4>((r_4)v.real(), (r_4)v.imag());
  ...
  //! Assignment operators FlagVector<16> (16 bits boolean array) 
  inline FlagVector16 operator= (FlagVector16& v)
  {
    if (typ_==DTC_FlagVec16Type) *(u_ptr_.b16)=v; 
    return v;
  }
  //! Assignment operators FlagVector<64> (64 bits boolean array) 
  inline FlagVector64 operator= (FlagVector64& v)
  {
    if (typ_==DTC_FlagVec64Type) *(u_ptr_.b64)=v; 
    return v;
  }

  //--- conversion to value operators (useful at right hand side of = ) 
  //! Conversion (cast) operator to char
  inline operator char() const 
  {
    if (typ_==DTC_CharacterType) return *(u_ptr_.cp);
    else return '\0';
  }
  ...
  inline operator int_4() const 
  {
    ...
    else if (typ_==DTC_CharacterType) return (int_4)(*(u_ptr_.cp)); 
    return 0;
  }
  ...
  inline operator int_8() const 
  {
    ...
    else if (typ_==DTC_CharacterType) return (int_8)(*(u_ptr_.cp)); 
    return 0;
  }
  ...
  inline operator std::complex<r_4> () const 
  {
    if (typ_==DTC_ComplexType) return *(u_ptr_.z4);
    else if (typ_==DTC_DoubleComplexType) return std::complex<r_4>(u_ptr_.z8->real(), u_ptr_.z8->imag());
    return std::complex<r_4>(0.,0.);
  }
  //! Conversion (cast) operator to double precision complex ( complex<r_8> )
  inline operator std::complex<r_8> () const 
  {
    if (typ_==DTC_ComplexType) return std::complex<r_8>(u_ptr_.z4->real(), u_ptr_.z4->imag());
  ...
  //! Conversion (cast) operator to FlagVec16
  inline operator FlagVector16 () const  
  {
    if (typ_==DTC_FlagVec16Type)  return(*(u_ptr_.b16)); 
    return FlagVector16() ;
  }
  //! Conversion (cast) operator to FlagVec16
  inline operator FlagVector64 () const  
  {
    if (typ_==DTC_FlagVec64Type)  return(*(u_ptr_.b64)); 
    return FlagVector64() ;
  }
...
public:
//! Constructor - For use by BaseDataTable class
  DataTableRowPtr( std::vector< DataTableCellType > const & coltypes, std::vector<std::string> const & colnames, std::vector<size_t> const & colvsz);
//! Copy constructor
...
//! Use of = operator is forbiden for DataTableRowPtr object : throws a ForbiddenError exception 
  DataTableRowPtr& operator = (DataTableRowPtr const & a)
  { throw ForbiddenError("DataTableRowPtr& operator = (DataTableRowPtr const & a) NOT permitted"); }
...
//! Copy content from the DataTableRowPtr \b a and column pairs identified by cid. No compatibility check is performed. 
  void   CopyContentFrom(DataTableRowPtr const & a, std::vector< std::pair<size_t, size_t> > const & cid);
//! prints the object content on the output stream \b os 
  std::ostream&   Print(std::ostream& os, const char* csep=" ; ", const char* vsep=",") const;
...
//! get cell/column index from its name - can throw NotFoundExc exception 
  size_t Index(std::string const & colname) const ;
...
//! return the DTCell object corresponding to element \b i of column \b k
  ...
  DTCell * cellptr_;
  std::vector< size_t > cellsizes_;      // cell vector size 
  std::map<std::string , size_t> nm2idx_;    // column/cell name to column index 
};

/*! Prints the DataTableRow object content on the output stream ( */
inline std::ostream& operator << (std::ostream& s, DataTableRowPtr const & row)
  {  row.Print(s);  return(s);  }
