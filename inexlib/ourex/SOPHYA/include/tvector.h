//-----------------------------------------------------------
// Classe TVector<T>
//    Vector class, inheriting from TMatrix<T>  
...
#include "tvector_tsnl.h"  /* For two level name look-up gcc >= 3.4 */
  ...
  TVector(const std::vector<T>& v, short lcv=BaseArray::AutoVectorType);
  ...
  sa_size_t FillFr(const std::vector<T>& v,bool noresize=false);
  //! Copy the std::vector<T> \b v content (to *this)  (alias for FillFr )
  inline sa_size_t CopyFrom(const std::vector<T>& v,bool noresize=false)
  { return FillFr(v,noresize) ; }
  sa_size_t FillTo(std::vector<T>& v,bool addtoend=false);
  //! Copy object content to the std::vector<T> \b v  (alias for FillTo )
  inline sa_size_t CopyTo(std::vector<T>& v,bool addtoend=false)
  { return FillTo(v, addtoend); }
  std::vector<T> ConvertTostdvec();
  ...
  // a copy of the sub-vector, as a packed vector
  TVector<T> PackAndCopySubMatrix(Range relt, short lcv=BaseArray::SameVectorType, short mm=BaseArray::SameMemoryMapping) const ;
...
/*! \ingroup TArray
  \typedef Vector
  \brief To simplify the most common usage of TVector<T> class, corresponding to TVector<r_8> (Vector of double) 
*/
...
extern template class TVector< std::complex<r_4> >;
extern template class TVector< std::complex<r_8> >;
#ifdef SO_LDBLE128
extern template class TVector<r_16>;
extern template class TVector< std::complex<r_16> >;
#endif
