...
/*! \brief Use string "s" to fill vector of strings "vs" considering char "sep" as a separator.
...
/*! \brief  Split the string "s" into a string vector "vs"
 ...
 That routine is much faster  than FillVStringFrString although less general
           (ex no '\' handling is done)
*/
{
 uint_4 ls = s.size();
 if(ls<=0) return;
