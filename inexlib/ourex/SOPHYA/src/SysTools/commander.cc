...
// ------------------------------------------------------------
//         Classes CmdExecutor2
// ------------------------------------------------------------

int CmdExecutor2::Execute(std::string& keyw, std::vector<std::string>& args, std::string& toks)
{
  CmdEx_ParamList plist;
  for (size_t i=0; i<args.size(); i++)  plist.add(args[i]);
  CmdEx_Param crv = Execute(keyw, plist);
  return crv.getRC();
}
...
usage += "  > listinterp    # list of available interpreters \n";
...
usage += "   ...  erf erfc j0 j1 \n";
usage += "   ...  gaussian(x,mu,x) gauss0(x)   \n";
usage += "   ...  rand01()  randpm1() randgauss(sigma, mean) gaurand() \n";
...
/*! 
  ...
  \param fgce2 : if true, registration of CmdExecutor2 object 
*/
void Commander::RegisterCommand(string& keyw, string& usage, CmdExecutor * ce, string& grp, bool fgce2)
{
if (!ce) { 
...
cmdexmap[keyw] = cme;
if (ce) 
  for (InterpMap::iterator it=interpmap.begin(); it!=interpmap.end(); it++) {
    if ((*it).second && ((*it).second != this))  (*it).second->RegisterCommandNotify(keyw);
  }
}
...
/* --Methode-- */
//! Add the command interpreter \b cl to the list of usable interpreters  
bool Commander::AddInterpreter(CmdInterpreter * cl)
{
  if (!cl) return false;
  InterpMap::iterator it= interpmap.find(cl->Name());
  if (it != interpmap.end())
    throw DuplicateIdExc("Commander::P_AddInterpreter() already registered interpreter with the same name");
  interpmap[cl->Name()] = cl;
  if (cl == this) return true;
  for (CmdExmap::iterator it=cmdexmap.begin(); it!=cmdexmap.end(); it++) {
    if ((*it).second.cex)  cl->RegisterCommandNotify((*it).first);
  }
  return true;
}
...
/* --Methode-- */
//! Removes the command interpreter \b cl from the list of usable interpreters  
void Commander::RemoveInterpreter(CmdInterpreter * cl)
{
  if (!cl) return;
  InterpMap::iterator it= interpmap.find(cl->Name());
  if (it != interpmap.end()) {
    if (curcmdi == (*it).second)  curcmdi=this;
    interpmap.erase(it);
  }
}
...
/* --Methode-- */
//! Select an interpreter by its name or its short name as the current interpreter. return true if required interpreter found
bool Commander::SelInterpreter(string const& name)
{
  bool fgok=false;
  for(InterpMap::iterator it=interpmap.begin(); it!=interpmap.end(); it++) {
    if (((*it).first==name)||((*it).second->ShortName()==name)) {
      cout << "Commander::SelInterpreter() - switching to command interpreter "<<name<<endl;
      curcmdi = (*it).second;    fgok=true;   break;
    }
  }
  return fgok;; 
}
...
/* --Methode-- */
//! Select the Commander as the current interpreter. 
void Commander::SelDefaultInterpreter()
{
  cout << "Commander::SelDefaultInterpreter() - switching to default command interpreter -> "<<Name()<<endl;
  curcmdi=this;  
}
...
else if (kw == "listinterp") {
  cout << "---- Commander::Interpret() - List of available interpreters ---"<<endl;
  int kki=0;
  for(InterpMap::iterator it=interpmap.begin(); it!=interpmap.end(); it++) {
    kki++;  cout<<"Interpreter["<<kki<<"] : Name= "<<(*it).second->Name()<< " ShortName= "<<(*it).second->ShortName();
    if (CurrentInterpreter() == (*it).second)  cout << " ** CurrentInterpreter ** " << endl;
    else cout << endl;
  }
  return(0);
}
...
/*! 
  \brief Execute the command identified by \b keyw with arguments args, through the registered CmdExecutors 
 
  This method handles command execution in a separate thread 
*/
/* --Methode-- */
...
/*! 
  ...
  This method handles command execution through the new/extended interface of CmdExecutor2 - 
  Execution in separate threads is not yet implemented 
*/
CmdEx_Param Commander::ExecuteCommand(string& keyw, CmdEx_ParamList & plist)
{
  //DBG  cout << "*DBG*Commander::ExecuteCommand(keyw="<<keyw<<" , plist) ->getNbParams()="<<plist.getNbParams()<<endl;
  CmdEx_Param rv;
  rv.setRC(9999);
  CmdExmap::iterator it = cmdexmap.find(keyw);
  if (it == cmdexmap.end())  {
    string errmsg="Commander::ExecuteCommand("+keyw+" ,): ERROR - No such command ";
    rv.setRC(90,errmsg);
    cerr<<errmsg<<endl;
    return rv;
  }
  if (! (*it).second.cex ) {
    string errmsg="Commander::ExecuteCommand("+keyw+" ,): ERROR - No CmdExecutor found ";
    rv.setRC(98,errmsg);
    cerr<<errmsg<<endl;
    return rv;
  }
  if ((*it).second.fgcex2) {
    CmdExecutor2 * cex2 = dynamic_cast<CmdExecutor2 *>((*it).second.cex);
    if (cex2) {
      //DBG      cout << "*DBG*Commander::ExecuteCommand() CmdExecutor2 found cex2="<<hex<<cex2<<dec<<endl;
      rv=cex2->Execute(keyw, plist);
      if (rv.getRC() != 0)
	cerr<<"Commander::ExecuteCommand("<<keyw<<",) Error, RC="<<rv.getRC()<<" : "<<rv.getErrorMsg()<<endl;
      return rv;
    }
    else {
      string errmsg="Commander::ExecuteCommand("+keyw+" ,): ERROR - Expected CmdExecutor2 / dynamic cast failed  ";
      rv.setRC(97,errmsg);
      cerr<<errmsg<<endl;
    }
  }
  ...
  string toks;
  for (size_t i=0; i<plist.getNbParams(); i++) {
    string s = plist.get(i).getValueS();
    args.push_back(s);    toks += " ";  toks += s;
  }
  //DBG  cout << "*DBG*Commander::ExecuteCommand() ... calling cex->Execute(keyw,args,toks) with args.size="<<args.size()<<" toks="<<toks<<endl; 
  
  int rc=(*it).second.cex->Execute(keyw, args, toks);
  rv.setRC(rc);
  return rv;
}
...
CmdInterpreter * mycinterp=CurrentInterpreter();
bool fgcinterp=false; 
int nline=0;
bool ohv = histon;
histon = false;
while (fgets(line_buff,1535,fip) != NULL)  {
  if (trace) ShowMessage(line_buff, _MAGENTA_);
  line_buff[1535]='\0';  nline++;
  size_t lbl=strlen(line_buff);
  if (lbl<1)  continue;  // on saute les lignes vides 
  line_buff[lbl-1] = '\0';   lbl--; /*  LF/CR de la fin */
  if ((nline==1)&&(lbl>2)&&(line_buff[0]=='#')&&(line_buff[1]=='!')) {  // selecting a specific interpreter
    for(size_t li=lbl-1; li>0; li--)  if ((line_buff[li]==' ')||(line_buff[li]=='\t'))  line_buff[li]='\0';
    string iname=line_buff+2;
    ShowMessage("### Switching to Interpreter ", _MAGENTA_);
    ShowMessage(iname.c_str(), _MAGENTA_);  ShowMessage("\n", _MAGENTA_);
    if (iname.length()>0)   fgcinterp=SelInterpreter(iname);
  }
  if ((lbl<1)||(line_buff[0]=='#'))  continue;  // on saute les lignes vides ou celles commencant par # 
  string line = line_buff;
  rcc = CurrentInterpreter()->Interpret(line);   
  if ((CurrentInterpreter() != this) && (rcc!=0))  break;
  if ((CurrentInterpreter() == this) && ((rcc == CMD_RETURN_RC) || (rcc == CMD_BREAKEXE_RC)) ) break;
 }
histon = ohv; 
...
if (fgcinterp)   {  // changing back to interpreter before change by the specification in the first line ...
  if (mycinterp==this)  SelDefaultInterpreter();
  else SelInterpreter(mycinterp->Name());
}
