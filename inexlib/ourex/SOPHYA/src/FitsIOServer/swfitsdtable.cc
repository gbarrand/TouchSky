...
/*!
   ...
   FitsInOutFile si("myswtable.fits", FitsInOutFile::Fits_RO);
				 ...
				 int colidx, vector<int_8> const * swpos, size_t vsz, bool fgfcnet)
{
  ...
  */
  ...
  {
    ser = mCCols.size();
    FITSDataSwapper<char> CSwapper(mSwF, colidx, vsz, this);
    if (swpos)
      mCCols.push_back(SwSegDataBlock<char>(CSwapper, *swpos, segsz, vsz));
    else 
      mCCols.push_back(SwSegDataBlock<char>(CSwapper, segsz, 0, vsz));
  ...
  {
    ser = mB16Cols.size();
    FITSDataSwapper< FlagVector16 > SSwapper(mSwF, colidx, vsz, this);
    if (swpos)    
      mB16Cols.push_back(SwSegDataBlock< FlagVector16 >(SSwapper, *swpos, segsz, vsz));
    else 
      mB16Cols.push_back(SwSegDataBlock< FlagVector16 >(SSwapper, segsz, 0, vsz));
  ...
  {
    ser = mB64Cols.size();
    FITSDataSwapper< FlagVector64 > BSwapper(mSwF, colidx, vsz, this);
    if (swpos)    
      mB64Cols.push_back(SwSegDataBlock< FlagVector64 >(BSwapper, *swpos, segsz, vsz));
    else 
      mB64Cols.push_back(SwSegDataBlock< FlagVector64 >(BSwapper, segsz, 0, vsz));
