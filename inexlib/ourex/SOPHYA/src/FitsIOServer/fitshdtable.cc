...
void FitsHandler<BaseDataTable>::Write(FitsInOutFile& os)
{
  ...
  for(sa_size_t k=0; k<dobj->NVar(); k++) { 
    ...
    case BaseDataTable::CharacterField :
      if (tbltyp == ASCII_TBL) {
	cout << "FitsHandler<BaseDataTable>::Write()/Warning CharacterField (SBYTE) not supported for ASCII_TBL " 
	     << endl;
	fgoknm = false;
      }
      else tform.push_back((str_tform+"S"));
      break;
    ...
    case BaseDataTable::FlagVec16Field :
      if (tbltyp == ASCII_TBL) {
	cout << "FitsHandler<BaseDataTable>::Write()/Warning FlagVec16Field not supported for ASCII_TBL " 
	     << endl;
      }
      else {
	sprintf(buff_tform,"%ldX",(long)(cvsz*16));
	tform.push_back(buff_tform);
	//DBG	cout << "*DBG*FitsHandler<BaseDataTable>::Write()/ FlagVec16Field - cvsz="<<cvsz<<" TFORM="<<buff_tform<<endl;
      }
      break;
      case BaseDataTable::FlagVec64Field :
      if (tbltyp == ASCII_TBL) {
	cout << "FitsHandler<BaseDataTable>::Write()/Warning FlagVec64Field not supported for ASCII_TBL " 
	     << endl;
      }
      else { 
	sprintf(buff_tform,"%ldX",(long)(cvsz*64));
	tform.push_back(buff_tform);
	//DBG	cout << "*DBG*FitsHandler<BaseDataTable>::Write()/ FlagVec64Field - cvsz="<<cvsz<<" TFORM="<<buff_tform<<endl;
      }
      break;
    default:
  ...
  // Ecriture des unites de colonnes , et identification des colonnes de type DateTimeField 
  size_t kk=0;
  for(sa_size_t k=0; k<dobj->NVar(); k++) {
    ...
    char keynb[32];
    if (dobj->GetColumnType(k) == BaseDataTable::DateTimeField) {
      sprintf(keynb,"SODTF%d",(int)kk);
      mtv = "DateTimeField";
      os.WriteKey(keynb,mtv," Column content = SOPHYA::DateTime"); 
    }
    bool sdone=false;
    Units un=dobj->GetUnits(k,sdone);
    if (!sdone) continue;
    sprintf(keynb,"SOPUN%d",(int)kk);
  ...
  for(sa_size_t l=0; l<dobj->NEntry(); l+=dobj->SegmentSize()) { 
	...
	break;
      case BaseDataTable::CharacterField :
	icol++;
	FitsBlockRW<char>::WriteColumnData(os, icol, l+1, 1, 
					   dobj->mCColsP[sk]->GetCstSegment(iseg), sz);
	break;
      case BaseDataTable::FMLStr16Field :
	icol++;
	...
	break;
      case BaseDataTable::FlagVec16Field :
	icol++;
	//DBG	cout << "*DBG*FitsHandler<BaseDataTable>::Write()FlagVector16-icol="<<icol<<" l="<<l
	//DBG     <<" sszt="<<sszt<<" sz="<<sz<< " segsize="<<dobj->SegmentSize()<<" NEntry="<<dobj->NEntry()<<endl;
	FitsBlockRW<FlagVector16>::WriteColumnData(os, icol, l+1, 1, 
						   dobj->mB16ColsP[sk]->GetCstSegment(iseg), sz);
	break;
      case BaseDataTable::FlagVec64Field :
	icol++;
	FitsBlockRW<FlagVector64>::WriteColumnData(os, icol, l+1, 1, 
						   dobj->mB64ColsP[sk]->GetCstSegment(iseg), sz);
	break;
      default:
	break;
      }
...
void FitsHandler<BaseDataTable>::Read(FitsInOutFile& is)
{
  ...
  for(sa_size_t k=0; k<colnames.size(); k++) { 
    ...
    // Checking if this column is of DateTimeField
    bool fgdatimcol=false;
    char keybuff[16];
    sprintf(keybuff,"SODTF%d",(int)k+1);
    string key=keybuff;
    bool nosk=false;
    string sodtf=is.KeyValue(key,nosk);
    if (!nosk&&(sodtf=="DateTimeField"))  fgdatimcol=true;
    ...
    case TLOGICAL :
      {
      size_t cvsz=1;	
      BaseDataTable::FieldType flgft=BaseDataTable::FlagVec16Field;
      if (repcnt[k]<56) { 
	flgft=BaseDataTable::FlagVec16Field;
	if (repcnt[k]>16)  cvsz=(repcnt[k]/16)+((repcnt[k]%16)==0?0:1);
      }
      else {
	flgft=BaseDataTable::FlagVec64Field;
	if (repcnt[k]>64)   cvsz=(repcnt[k]/64)+((repcnt[k]%64)==0?0:1);
      }
      if (swfdt) 
	swfdt->AddColRd(flgft, colnames[k], k+1, &swpos, cvsz);
      else dobj->AddColumn(flgft, colnames[k], cvsz);
      colpos.push_back(k+1);
      //DBG      cout << "FitsHandler<BaseDataTable>::Read()**DBG*  TBIT/TLOGICAL, coltypes="<<coltypes[k]
      //DBG	   <<" repcnt="<<repcnt[k]<<" cvsz="<<cvsz<<endl;
      }
      break;  
    case TSHORT :
    case TUSHORT :
    ...
    case TDOUBLE :
      if (swfdt) { 
	if (fgdatimcol)  swfdt->AddColRd(BaseDataTable::DateTimeField, colnames[k], k+1, &swpos, repcnt[k]);   
	else  swfdt->AddColRd(BaseDataTable::DoubleField, colnames[k], k+1, &swpos, repcnt[k]); 
      }    
      else {
	if (fgdatimcol) dobj->AddDateTimeColumn(colnames[k], repcnt[k]);
	...
	if (swfdt)       
      ...
      if (swfdt)       
	swfdt->AddColRd(BaseDataTable::StringField, colnames[k], k+1, &swpos, 1);     
      else dobj->AddStringColumn(colnames[k], 1);
      }
      colpos.push_back(k+1);
      break;
    case TBYTE :
    case TSBYTE :
      if (swfdt) 
	swfdt->AddColRd(BaseDataTable::CharacterField, colnames[k], k+1, &swpos, repcnt[k]);
      else dobj->AddCharacterColumn(colnames[k], repcnt[k]);
      colpos.push_back(k+1);
      break;
    default:
      cerr << "FitsHandler<BaseDataTable>::Read() NOT handled field type " 
	   << coltypes[k] << endl;
      break;
    }
  }
  ...
  while ((dobj->SegmentSize()*dobj->NbSegments()) < dobj->NEntry())  dobj->Extend();
  ...
  for(sa_size_t l=0; l<dobj->NEntry(); l+=dobj->SegmentSize()) { 
	...
	break;
      case BaseDataTable::CharacterField :
	FitsBlockRW<char>::ReadColumnData(is, icol, l+1, 1, 
					  dobj->mCColsP[sk]->GetSegment(iseg), sz);
	break;
      case BaseDataTable::FMLStr16Field :
	...
	break;
      case BaseDataTable::FlagVec16Field :
	//DBG	cout << "*DBG*FitsHandler<BaseDataTable>::Read()FlagVector16-icol="<<icol<<" l="<<l
	//DBG     <<" sszt="<<sszt<<" sz="<<sz<< " segsize="<<dobj->SegmentSize()<<" NEntry="<<dobj->NEntry()<<endl;
	FitsBlockRW<FlagVector16>::ReadColumnData(is, icol, l+1, 1, 
						  dobj->mB16ColsP[sk]->GetSegment(iseg), sz);
	break;
      case BaseDataTable::FlagVec64Field :
	FitsBlockRW<FlagVector64>::ReadColumnData(is, icol, l+1, 1, 
						  dobj->mB64ColsP[sk]->GetSegment(iseg), sz);
	break;
      default:
	break;
      }
