...
void FITS_SphereHEALPix<T>::WriteToFits(FitsOutFile& os) 
{
  ...
  */
  ...
  int nSide= dobj_->getNSide();  // =SizeIndex();
  ...
  if (typeid(T) == typeid(r_8) ) Type+='D';
      ...
      throw FitsIOException("FITS_SphereHEALPix<T>::WriteToFits() unsupported data type");
    }
  vector<int> dummy;
  os.makeHeaderBntblOnFits(Type, Noms, nPix, 1, &dvl, extname, dummy);
  os.PutColToFits(0,  nPix, dobj_->DataBlock().Data()); 
} 
...
void FITS_SphereHEALPix<T>::ReadFromFits(FitsInFile& is)
{
  ...
  if (nbcols != 1) 
    cerr<<"FITS_SphereHEALPix<T>::ReadFromFits()/WarningFits::NbCols="<<nbcols<<" != 1 !" << endl;
  ...
  */
  ...
  int nside= dvl.GetI("NSIDE",-1);
  if (nside<1) 
    throw FitsIOException("FITS_SphereHEALPix<T>::ReadFromFits() No resol parameter NSIDE");

  if(dobj_ == NULL)  {
    dobj_= new SphereHEALPix<T>(nside, fgring);
    ownobj_= true;      
  }
  else dobj_->setNSide(nside, fgring);
  if (nbentries != dobj_->NbPixels()) 
    throw FitsIOException("FITS_SphereHEALPix<T>::ReadFromFits() nbentries != dobj_->NbPixels()");
  dobj_->Info()  = is.DVListFromFits();

  is.GetBinTabFCol(dobj_->DataBlock().Data(),dobj_->NbPixels(),0);  
}
