...
#ifdef cfitsio_ourex_cfitsio_h
inline int fits_free_memory(void* value,int  *status) {
...
/*! 
  ...
  This class encapsulates the most common services provided by the c-fitsio library :
  http://heasarc.gsfc.nasa.gov/docs/software/fitsio/fitsio.html
  which are used by the SOPHYA objects fits handlers. The most common use of this class 
  is through instanciating it to open an existing fits file, to read objects from, 
  or for creating a new file and writing objects to it. the stream operators push (<<) 
  and pull (>>) operators can be used for SOPHYA objects which have a FITS handler. 

  \sa SOPHYA::FitsBlockRW

  \code 
  #include "fitsioserver.h"
  // ...
  {
  // ---- Writing an array to a fits file
  TArray<int_4> arr(7, 12); 
  arr = RegularSequence(0.,5.);
  FitsInOutFile fos("!marr.fits", FitsInOutFile::Fits_Create);
  fos << arr;
  }
  {
  //---- Reading from the fits file
  TArray<int_4> arr;
  FitsInOutFile fis("marr.fits", FitsInOutFile::Fits_RO);
  fis >> arr;
  cout << arr; 
  }
  \endcode 
  ...
  \warning 
  HDU numbers start at 1, following cfitsio convention 
*/
...
void FitsInOutFile::Close()
{
  ...
  int status = 0;
  int nbhdus=0;
  if ((mode_==Fits_Create)||(mode_==Fits_RW)) {
    status = 0;
    fits_get_num_hdus(FitsPtr(), &nbhdus, &status);
    //DBG    cout << "FitsInOutFile::Close()*DBG* fits_get_num_hdu()->nbhdus="<<nbhdus<<" status="<<status<<endl;
    if (status) {
      cerr << " FitsInOutFile::Close FileCreate/Error fits_get_num_hdus !" << endl;
      fits_report_error(stderr, status);
    }
    else if ((nbhdus>0)&&(mode_==Fits_Create)) {
      status = 0;
      int hdutyp;
  ...
  if (nbhdus<1) 
    cerr<<"FitsInOutFile::Close()/Warning: closing empty file (nbhdus="<<nbhdus<<")"<<endl;
...
void FitsInOutFile::InsertColumn(int numcol, const char* colname, const char* fmt)
{
  ...
  FitsCheckStatus(status, "FitsInOutFile::InsertColumn() Error: ");

  return;
}

/*-- Methode --*/
string FitsInOutFile::GetHeader(int & nkeyrec, bool fgnoch) const

/*! 
  the 80 character long header records are concatenated into a character array and returned as a std::string.
  No newline or linefeed character inserted between records. Call the cfitsio routine fits_hdr2str() . 
...
*/
{
  ...
  FitsCheckStatus(status, "FitsInOutFile::GetHeader() fits_hdr2str() Error: ");
  string rs=header;
  fits_free_memory(header, &status);  
  FitsCheckStatus(status, "FitsInOutFile::GetHeader() fits_free_memory() Error: ");
  return rs;
}
...
int FitsInOutFile::GetHeaderRecords(DVList& dvl, bool stripkw, bool keepstkey)
{
  ...
  for(int kk=1; kk<=nkeys; kk++) {
    ...
    case 'L' :   // valeurs logiques  TRUE / FALSE 
      if (value[0]=='T')   dvl.SetB(keyname, true);
      else  dvl.SetB(keyname, false);
      break;
    case 'I' :   // Entiers 
			     ...
			     const char *comment)
{
  ...
  case MuTyV::MTVBoolean :
    mtv.Convert(bval);  bvali=(bval?1:0);
    fits_write_key(FitsPtr(), TLOGICAL, keyname, &bvali, comm, &status);
    break;
...
/*-- Methode --*/
void FitsInOutFile::PrintHeader(ostream& os, bool fgnoch) const
{
  int status=0 , nkeyrec=0;;
  char * header=NULL;
  fits_hdr2str(FitsPtr(), (fgnoch?1:0), NULL, 0, &header, &nkeyrec, &status);
  FitsCheckStatus(status, "FitsInOutFile::PrintHeader() fits_hdr2str() Error: ");
  if (nkeyrec>1) {
    char c=header[0];
