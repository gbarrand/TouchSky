...
*/
template<class T>
T LocalMap<T>::PixVal(int_4 k) const
{
  if((k < 0) || (k >= nPix_)) 
    {
    throw RangeCheckError("LocalMap::PIxVal Pixel index out of range ");
  ...
  int it, ip;
   for (it = 0; it < nSzY_; it++)
      ...
      for (ip = 0; ip < nSzX_; ip++) 
	{
	  ...
	  sphere.PixValSph(theta,phi)= pixels_(it, ip);
	}
    }
}
...
template class LocalMap<int_8>;
template class LocalMap<r_8>;
template class LocalMap<r_4>;
template class LocalMap< std::complex<r_8> >;
template class LocalMap< std::complex<r_4> >;
#ifdef SO_LDBLE128
template class LocalMap<r_16>;
template class LocalMap< std::complex<r_16> >;
#endif
}
#endif
