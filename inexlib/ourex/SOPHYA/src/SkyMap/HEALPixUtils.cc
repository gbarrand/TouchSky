...
// Instance unique de la classe HPU_PIXELS_XY 
static HPU_PIXELS_XY * _singleton = NULL;

HPU_PIXELS_XY& HPU_PIXELS_XY::instance()
{
  if (_singleton == NULL) _singleton = new HPU_PIXELS_XY ;
  return (*_singleton);
}

void HPU_PIXELS_XY::mk_pix2xy() 
{
  /*
    ==================================================
...
int_4 HEALPixUtils::nest2ring(int_4 nside, int_4 ipnest) 
{
  /*
    ====================================================
  ...
  */
  ...
  //  (16/12/98)
    ...
    const HPU_PIXELS_XY& PXY= HPU_PIXELS_XY::instance();
  ...
  if(  nside<1 ||  nside>ns_max ) {
    char buff[64];
    sprintf(buff,"HEALPixUtils::nest2ring nside(=%d) out of range ", nside);
    throw RangeCheckError(PExcLongMessage(buff));
  }
  npix = 12 *  nside* nside;
  if( ipnest<0 || ipnest>npix-1 ) {
    char buff[64];
    sprintf(buff,"HEALPixUtils::nest2ring ipnest(=%d) out of range ", ipnest);
    throw RangeCheckError(PExcLongMessage(buff));
  }
...
int_4 HEALPixUtils::ring2nest(int_4 nside, int_4 ipring)  
{
  /*
    ==================================================
  ...
  */
  ...
  if( nside<1 || nside>ns_max ) {
    char buff[64];
    sprintf(buff,"HEALPixUtils::ring2nest nside(=%d) out of range ", nside);
    throw RangeCheckError(PExcLongMessage(buff));
  }
  npix = 12 * nside*nside;
  if( ipring<0 || ipring>npix-1 ) {
    char buff[64];
    sprintf(buff,"HEALPixUtils::ring2nest ipring(=%d) out of range ", ipring);
    throw RangeCheckError(PExcLongMessage(buff));
  }
...
int_4 HEALPixUtils::ang2pix_ring(int_4 nside, double theta, double phi)  
{
  /*
    ==================================================
  ...
  */
  ...
  if( nside<1 || nside>ns_max ) {
    char buff[64];
    sprintf(buff,"HEALPixUtils::ang2pix_ring nside(=%d) out of range ", nside);
    throw RangeCheckError(PExcLongMessage(buff));
  }

  if( theta<0. || theta>Pi) {
    char buff[64];
    sprintf(buff,"HEALPixUtils::ang2pix_ring theta(=%g) out of range ", theta);
    throw RangeCheckError(PExcLongMessage(buff));
  }
...
int_4 HEALPixUtils::ang2pix_nest(int_4 nside, double theta, double phi) 
{
  /*
    ==================================================
  ...
  */
  ...
  if( nside<1 || nside>ns_max ) {
    char buff[64];
    sprintf(buff,"HEALPixUtils:::ang2pix_nest nside(=%d) out of range ", nside);
    throw RangeCheckError(PExcLongMessage(buff));
  }
  if( theta<0 || theta>Pi ) {
    char buff[64];
    sprintf(buff,"HEALPixUtils:::ang2pix_nest theta(=%g) out of range ", theta);
    throw RangeCheckError(PExcLongMessage(buff));
  }
  z  = cos(theta);
  za = fabs(z);
  ...
  if( za<=z0 ) { // then ! equatorial region
    ...
    jp = (int)std::min(ns_max-1, jp);// ! for points too close to the boundary
    jm = (int)std::min(ns_max-1, jm);
...
void HEALPixUtils::pix2ang_ring(int_4 nside,int_4 ipix,double& theta,double& phi)  
{
  /*
    ===================================================
  ...
  */
  ...
  if( nside<1 || nside>ns_max ) {
    char buff[64];
    sprintf(buff,"HEALPixUtils::pix2ang_ring nside(=%d) out of range ", nside);
    throw RangeCheckError(PExcLongMessage(buff));
  }
  npix = 12*nside*nside;      // ! total number of points
  if( ipix<0 || ipix>npix-1 ) {
    char buff[64];
    sprintf(buff,"HEALPixUtils::pix2ang_ring ipix(=%d) out of range ", ipix);
    throw RangeCheckError(PExcLongMessage(buff));
  }
...
void HEALPixUtils::pix2ang_nest(int_4 nside,int_4 ipix,double& theta,double& phi)  
{
  /*
    ==================================================
  ...
  */
  ...
  if( nside<1 || nside>ns_max ) {
    char buff[64];
    sprintf(buff,"HEALPixUtils::pix2ang_nest nside(=%d) out of range ", nside);
    throw RangeCheckError(PExcLongMessage(buff));
  }
  npix = 12 * nside*nside;
  if( ipix<0 || ipix>npix-1 ) {
    char buff[64];
    sprintf(buff,"HEALPixUtils::pix2ang_nest ipix(=%d) out of range ", ipix);
    throw RangeCheckError(PExcLongMessage(buff));
  }
...
/*! 
  \brief return the size index value (nside) corresponding to resolution \b res  
  Computes the size index (nside) providing a resolution better or equal the 
  ...
  \sa SOPHYA::Angle
*/
int_4 HEALPixUtils::ResolToSizeIndex(double res)
{
  int_4 m = 1;
...
//! return the pixel resolution for a given HEALPixUtils size index (nside)
double HEALPixUtils::SizeIndexToResol(int_4 m)
{
