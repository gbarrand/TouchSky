...
public:
  ...
  virtual void   WriteASCII(std::ostream& os) const = 0;
...
protected:
  ...
  bool UpdateSizes(int_4 ndim, const sa_size_t * siz, sa_size_t step, sa_size_t offset, std::string & exmsg);
  bool UpdateSizes(int_4 ndim, const sa_size_t * siz, const sa_size_t * step, sa_size_t offset, std::string & exmsg);
  bool UpdateSizes(const BaseArray& a, std::string & exmsg);
  //! return the size (number of data elements) of NDataBlock needed to hold the data \sa ComputeTotalSize() 
  sa_size_t ComputeDataBlockSize() const;
  //! Compute the total size (number of data elements) needed to hold the data with the layout specified by the parameters 
...
{
  if ( (ka < 0) || (ka >= ndim_) ) {
    std::string txt = "BaseArray::CheckDimensionIndex/Error ";   txt += ck_op_msg_[msg];
...
inline void BaseArray::CheckBound(sa_size_t ix, sa_size_t iy, sa_size_t iz, sa_size_t it, sa_size_t iu, int msg)  const
{
  if ( (ix >= size_[0]) || (ix < 0) || (iy >= size_[1]) || (iy < 0) || 
       ...
       (iu >= size_[4]) || (iu < 0) )  {
    std::string txt = "BaseArray::CheckArrayBound/Error ";   txt += ck_op_msg_[msg];
    throw(RangeCheckError(txt));
  }
  return;
}
