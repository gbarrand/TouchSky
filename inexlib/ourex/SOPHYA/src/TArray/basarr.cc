//-----------------------------------------------------------
// Classe BaseArray : Base class for numerical arrays 
//      Base array class - Memory organisation management
// SOPHYA class library - (C)  UPS+LAL IN2P3/CNRS  , CEA-Irfu 
// R. Ansari          (C) UPS+LAL IN2P3/CNRS  2003
// C. Magneville      (C) DAPNIA/SPP  CEA     2003
//-----------------------------------------------------------
			     ...
			     sa_size_t& stepa, sa_size_t& gpas, sa_size_t& naxa) const
{
  if (smo) { // Same memory organisation
    // Petit changement de strategie de parcours du tableau - dans un but d'optimisation - Reza, Avril 2016 
    int_4 mnstka=MinStepKA();
    int_4 mxszka=MaxSizeKA();
    if ((Step(mnstka)==1)&&(Size(mnstka)>1)&&(Size(mnstka)>=Size(mxszka)/10))  ax=axa=mnstka;
    else if ((Size(mnstka)>=Size(mxszka)/2)&&(Step(mnstka)<=Step(mxszka)/2))   ax=axa=mnstka;
    else ax=axa=mxszka;
  }
  else {
...
sa_size_t BaseArray::ComputeDataBlockSize() const
{
  if (totsize_<1)  return 0;
  sa_size_t rs = offset_+1;
  for(int_4 k=0; k<ndim_; k++)   rs += (size_[k]-1)*step_[k];
  sa_size_t rs2=0;
  //  $CHECK$ --  Reza , Avril 2016 , Pour verification - a supprimer 
  if ((moystep_>0) && (rs != (rs2=ComputeTotalSize(ndim_,size_,moystep_,offset_))))  {
    cout<<" BaseArray::ComputeDataBlockSize() - BUG ?? rs="<<rs<<" rs2="<<rs2<<endl;
    Show();
    throw PError("BUG BaseArray::ComputeDataBlockSize()");
  }
  return rs;
}
