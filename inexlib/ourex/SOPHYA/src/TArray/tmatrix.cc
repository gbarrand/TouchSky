...
/*! 
  \brief Return a sub-matrix defined by the range of rows \b rline and range of columns \b rcol
  
  \warning data is shared between the original matrix and the sub-matrix
  \param rline : range of rows
  \param rcol : range of columns 
*/
...
/*! 
  \brief Return a packed copy of the sub-matrix defined by the range of rows \b rline and range of columns \b rcol, as a packed matrix

  matrix data is copied in all cases, even if the defined sub-matrix is a packed array. 

  \param rline : range of rows
  \param rcol : range of columns 
  \param mm : MemoryMapping of the returned matrix  
*/

template <class T>
TMatrix<T> TMatrix<T>::PackAndCopySubMatrix(Range rline, Range rcol, short mm) const
{
  Range rx=Range::first();
  Range ry=Range::first();
  short mmo = GetMemoryMapping();
  if (mmo == CMemoryMapping)  { rx = rcol;  ry = rline; }
  else { ry = rcol;  rx = rline; }
  TMatrix sm(SubArray(rx, ry, Range::first(), Range::first(), Range::first(), false), true);
  sm.UpdateMemoryMapping(mmo);
  if ((mm!=BaseArray::CMemoryMapping)&&(mm!=BaseArray::FortranMemoryMapping))  mm=GetMemoryMapping();
  TMatrix rsm( sm.NRows(), sm.NCols(), mm, false);
  rsm=sm;
  return(rsm);
}
...
ostream& TMatrix<T>::Print(ostream& os, sa_size_t maxprt, bool si, bool ascd) const
{
  ...
  else if ( typeid(T) == typeid(complex<r_4>) ) prtw = 2*fprtw;
  else if ( typeid(T) == typeid(complex<r_8>) ) prtw = 2*fprtw;
...
//           Fortan-like matrices are line   packed
{
  ...
  if( (popt&(uint_2)4) ) {
    ...
    if(totsiza<totsizb) {   // on copie A
      ...
      TMatrix<T> _bcopy(b.NRows(),b.NCols(),BaseArray::FortranMemoryMapping);
      _bcopy = b;
      rm = Multiply(_bcopy,mm);
    }
    return rm;
  }
