...
/*!
  ...
  \param step : step along the first dimension with size > 1 (equaly spaced elements)
...
/*!
  ...
  \param step : step (along the first dimension with size>1) in data block
  \param offset : offset for first element in data block
 */
template <class T>
TArray<T>::TArray(int_4 ndim, const sa_size_t * siz, const NDataBlock<T> & db, bool share, sa_size_t step, sa_size_t offset)
...
/*!
  ...
  \param offset : offset for first element in data block
  \param br : if not NULL, Bridge object handling the external data pointer
  \sa NDataBlock
 */
template <class T>
...
TArray<T>& TArray<T>::Set(const TArray<T>& a)
{
  ...
  if (a.NbDimensions() < 1) 
    throw NotAllocatedError("TArray<T>::Set(a ) - Array a not allocated ! ");
...
/*! 
  \brief Set array elements equal to the \b a array elements, after conversion 
  
  Non allocated arrays will be allocated with the same memory layout as the \b a array
*/
template <class T>
TArray<T>& TArray<T>::SetBA(const BaseArray& a)
{
  if (this == &a) return(*this);
  if (a.NbDimensions() < 1) 
    throw NotAllocatedError("TArray<T>::SetBA(a ) - Array a not allocated ! ");
  if (NbDimensions() < 1) {
    string exmsg = "TArray<T>::SetBA(const BaseArray& a)";
    if (!UpdateSizes(a, exmsg))  throw( ParmError(exmsg) );
    mNDBlock.ReSize(ComputeDataBlockSize(), false);    
  }
  ConvertAndCopyElt(a);
  return(*this);
}
...
void TArray<T>::ReSize(int_4 ndim, sa_size_t * siz, sa_size_t step, bool fzero)
{
  ...
  if (!UpdateSizes(ndim, siz, step, 0, exmsg))  throw( ParmError(exmsg) );
  mNDBlock.ReSize(ComputeDataBlockSize(), fzero);    
}
...
void TArray<T>::Realloc(int_4 ndim, sa_size_t * siz, sa_size_t step, bool force)
{
  ...
  if (!UpdateSizes(ndim, siz, step, 0, exmsg))  throw( ParmError(exmsg) );
  mNDBlock.Realloc(ComputeDataBlockSize(), force);    
}

/*!
  Convenience function for resizing the array, creating a one dimensional array
  \param nx : 1D array size 
  \param fzero : if true, sets all elements to 0 
*/
template <class T>
void TArray<T>::SetSize1D(sa_size_t nx, bool fzero)
{
  sa_size_t mysize[BASEARRAY_MAXNDIMS]={0,0,0,0,0};
  mysize[0]=nx;
  ReSize(1, mysize, 1, fzero);
  return;
}

/*!
  Convenience function for resizing the array, creating a two dimensional array
  \param nx , ny : 2D array sizes  
  \param fzero : if true, sets all elements to 0 
*/
template <class T>
void TArray<T>::SetSize2D(sa_size_t nx, sa_size_t ny, bool fzero)
{
  sa_size_t mysize[BASEARRAY_MAXNDIMS]={0,0,0,0,0};
  mysize[0]=nx;  mysize[1]=ny; 
  ReSize(2, mysize, 1, fzero);
  return;
}

/*!
  Convenience function for resizing the array, creating a three dimensional array
  \param nx , ny , nz : 3D array sizes  
  \param fzero : if true, sets all elements to 0 
*/
template <class T>
void TArray<T>::SetSize3D(sa_size_t nx, sa_size_t ny, sa_size_t nz, bool fzero)
{
  sa_size_t mysize[BASEARRAY_MAXNDIMS]={0,0,0,0,0};
  mysize[0]=nx;  mysize[1]=ny;  mysize[2]=nz; 
  ReSize(3, mysize, 1, fzero);
  return;
}

/*!
  Convenience function for resizing the array, creating a four dimensional array
  \param nx , ny , nz , nt : 4D array sizes  
  \param fzero : if true, sets all elements to 0 
*/
template <class T>
void TArray<T>::SetSize4D(sa_size_t nx, sa_size_t ny, sa_size_t nz, sa_size_t nt, bool fzero)
{
  sa_size_t mysize[BASEARRAY_MAXNDIMS]={0,0,0,0,0};
  mysize[0]=nx;  mysize[1]=ny;  mysize[2]=nz;  mysize[3]=nt; 
  ReSize(4, mysize, 1, fzero);
  return;
}

/*!
  Convenience function for resizing the array, creating a four dimensional array
  \param nx , ny , nz , nt , nu : 5D array sizes  
  \param fzero : if true, sets all elements to 0 
*/
template <class T>
void TArray<T>::SetSize5D(sa_size_t nx, sa_size_t ny, sa_size_t nz, sa_size_t nt, sa_size_t nu, bool fzero)
{
  sa_size_t mysize[BASEARRAY_MAXNDIMS]={0,0,0,0,0};
  mysize[0]=nx;  mysize[1]=ny;  mysize[2]=nz;  mysize[3]=nt;  mysize[4]=nu; 
  ReSize(5, mysize, 1, fzero);
  return;
}
...
MuTyV & TArray<T>::ValueAtPosition(sa_size_t ip) const
{
  ...
  if ( (ip >= totsize_) || (ip < 0) )   
    throw( RangeCheckError("TArray<T>::ValueAtPosition(sa_size_t ip) Out-of-bound Error") );
#endif
...
MuTyV & TArray<T>::ValueAtPositionDB(sa_size_t ip) const
{
  ...
  if ( (ip >= mNDBlock.Size() ) || (ip < 0) )   
    throw( RangeCheckError("TArray<T>::ValueAtPositionDB(sa_size_t ip) Out-of-bound Error") );
#endif
...
{
  if (NbDimensions() < 1) 
    throw NotAllocatedError("TArray<T>::PackElements() - Not Allocated Array ! ");
  if ( !force && (AvgStep() == 1) ) {
    TArray<T> ra;
    ra.Share(*this);
    return(ra);
...
/*!
  \brief Return a reshaped array, with the specified dimensions, with the data from the original arrays.   

  \param nx,ny,nz,nt,nu : sizes along first, second, third, fourth and fifth dimension 
    array rank is determined by the number of consecutive non zero dimensions (starting from nx)
  \param  share : if true, data is shared between the original array and the returned array  

  \warning throws an exception is data sharing is requested (share=true) and the original array is not packed 
 */

template <class T>
TArray<T> TArray<T>::ReShape(sa_size_t nx, sa_size_t ny, sa_size_t nz, sa_size_t nt, sa_size_t nu, bool share)
{
  sa_size_t size[BASEARRAY_MAXNDIMS];
  size[0] = nx;   size[1] = ny;   size[2] = nz;
  size[3] = nt;   size[4] = nu;
  ...
  if (!IsPacked() && share)  throw  ParmError("TArray<T>::ReShape(share=true) Unpacked array - Cannot reshape it sharing the data ...");
  if (IsPacked())  return TArray<T>(ndim, size, DataBlock(), share);
  else return TArray<T>(ndim, size, PackElements().DataBlock(), true);
}
...
/*!
  \brief Extract a sub-array defined by the range of elements along the different dimensions 
...
{
  if (NbDimensions() < 1) 
    throw NotAllocatedError("TArray<T>::SubArray(Range, ...) - Not Allocated Array ! ");
  int_4 ndim = 0;
...
/*!
  \brief Returns a copy of the sub-array defined by the ranges, as a packed array  

  array data is copied in all cases, even if the defined sub-array is a packed array. 
  \param rx,ry,rz,rt,ru : ranges of extraction along the 5 dimensions (X,Y,Z,T,U) of the array 

  \sa SOPHYA::Range
 */
template <class T>
TArray<T> TArray<T>::PackAndCopySubArray(Range rx, Range ry, Range rz, Range rt, Range ru) const
{
  if (NbDimensions() < 1) 
    throw NotAllocatedError("TArray<T>::PackAndCopySubArray(Range, ...) - Not Allocated Array ! ");

  TArray<T> sa=SubArray(rx, ry, rz, rt, ru, true);  
  // Updating Range objects using actual array size
...
{
  if (NbDimensions() < 1) 
    throw NotAllocatedError("TArray<T>::SetSeq(Sequence ) - Not Allocated Array ! ");
...
{
  if (NbDimensions() < 1) 
    throw NotAllocatedError("TArray<T>::SetCst(T )  - Not Allocated Array ! ");
  T * pe;
  sa_size_t j,k;
...
{
  if (NbDimensions() < 1) 
    throw NotAllocatedError("TArray<T>::AddCst(T,res)  - Not allocated source array ");
...
{
  if (NbDimensions() < 1) 
    throw NotAllocatedError("TArray<T>::SubCst(T,res)  - Not allocated source array ");
  if (res.NbDimensions() < 1)  {
    if ( IsTemp() )  res.Share(*this);
    else res.SetSize(*this, true, false);
  }
...
{
  if (NbDimensions() < 1) 
    throw NotAllocatedError("TArray<T>::MulCst(T,res)  - Not allocated source array ");
  if (res.NbDimensions() < 1)  {
    if ( IsTemp() )  res.Share(*this);
    else res.SetSize(*this, true, false);
  }
...
{
  if (NbDimensions() < 1) 
    throw NotAllocatedError("TArray<T>::DivCst(T,res)  - Not allocated source array ! ");
  if (!fginv && (x == (T) 0) ) 
...
{
  if (NbDimensions() < 1) 
    throw NotAllocatedError("TArray<T>::NegateElt(res)  - Not allocated source array ");
  if (res.NbDimensions() < 1)  {
    if ( IsTemp() )  res.Share(*this);
    else res.SetSize(*this, true, false);
  }
...
template <class T>
TArray<T>& TArray<T>::NegateEltSelf()
{
  if (NbDimensions() < 1) 
    throw NotAllocatedError("TArray<T>::NegateEltSelf()  - Not Allocated Array ! ");
  T * pe;
  sa_size_t j,k;
  if (AvgStep() > 0)   {  // regularly spaced elements
    sa_size_t step = AvgStep();
...
/*!
  \param pack : if \b true: create and return a packed array, \b false: same memory layout as the source array 
  \return Return a new array filled with the square of the original array elements
*/
template <class T>
TArray<T> TArray<T>::SquareElt(bool pack) const
{
  if (NbDimensions() < 1) 
    throw NotAllocatedError("TArray<T>::SquareElt(bool pack)  - Not allocated source array ");
  TArray<T> res;
  res.ReSize(*this, pack, false);
  bool smo;
  if (!CompareSizes(res, smo))   // Ca ne devrait pas arriver ...
    throw(SzMismatchError("TArray<T>::SquareElt(res) SizeMismatch(this,res) BUG ?")) ;
  ...
  if (smo && (IsPacked() > 0) && (res.IsPacked() > 0))   {  // regularly spaced elements
    ...
    for(j=0; j<naxa; j++)  {
      pe = mNDBlock.Begin()+Offset(ax,j);
      per = res.DataBlock().Begin()+res.Offset(axr,j);
      for(k=0;  k<gpas;  k+=step, pe+=step, per+=stepr)  *per = (*pe)*(*pe); 
    }
  }
  return(res);
}
...
template <class T>
TArray<T>& TArray<T>::SquareEltSelf()
{
  if (NbDimensions() < 1) 
    throw NotAllocatedError("TArray<T>::SquareEltSelf()  - Not Allocated Array ! ");
  T * pe;
  sa_size_t j,k;
  if (AvgStep() > 0)   {  // regularly spaced elements
    sa_size_t step = AvgStep();
    sa_size_t maxx = totsize_*step;
    pe = Data();
    for(k=0; k<maxx; k+=step )  pe[k] = (pe[k]*pe[k]);
  }
  else {    // Non regular data spacing ...
    int_4 ka = MaxSizeKA();
    sa_size_t step = Step(ka);
    ...
    for(j=0; j<naxa; j++)  {
      pe = mNDBlock.Begin()+Offset(ka,j);
      for(k=0; k<gpas; k+=step)  pe[k] = (pe[k]*pe[k]);
    }
  }
  return *this;
}
...
{
  if (NbDimensions() < 1) 
    throw NotAllocatedError("TArray<T>::AddElt(...) - Not allocated source array ! ");
  bool smoa;
...
\param a : Array to be subtracted from the source array.
\param res : Output array containing the result (res=*this+x). 
...
{
  if (NbDimensions() < 1) 
    throw NotAllocatedError("TArray<T>::SubElt(...) - Not allocated source array ! ");
  bool smoa;
  if (!CompareSizes(a, smoa)) 
...
\param a : Array to be multiplied to the source array.
\param res : Output array containing the result (res=(*this)*a). 
*/
template <class T>
...
{
  if (NbDimensions() < 1) 
    throw NotAllocatedError("TArray<T>::MulElt(...) - Not allocated source array ! ");
  bool smoa;
  if (!CompareSizes(a, smoa)) 
...
\param a : Array to which divide the source array - second (or first if fginv=true) argument of the operation.
\param res : Output array containing the result (res=*this/a). 
...
{
  if (NbDimensions() < 1) 
    throw NotAllocatedError("TArray<T>::DivElt(...) - Not allocated source array ! ");
  bool smoa;
  if (!CompareSizes(a, smoa)) 
...
{
  if (NbDimensions() < 1) 
    throw NotAllocatedError("TArray<T>::CopyElt(const TArray<T>& )  - Not Allocated Array ! ");
  bool smo;
...
{
  if (NbDimensions() < 1) 
    throw NotAllocatedError("TArray<T>::ConvertAndCopyElt(const TArray<T>& )  - Not Allocated Array ! ");
  bool smo;
  if (!CompareSizes(a, smo)) 
...
{
  if (NbDimensions() < 1) 
    throw NotAllocatedError("TArray<T>::ScalarProduct(...)  - Not allocated source array ");
  bool smo;
  if (!CompareSizes(a, smo)) 
...
{
  if (NbDimensions() < 1) 
    throw NotAllocatedError("TArray<T>::Sum()  - Not Allocated Array ! ");
  T ret=0;
  const T * pe;
  sa_size_t j,k;
  if (AvgStep() > 0)   {  // regularly spaced elements
...
{
  if (NbDimensions() < 1) 
    throw NotAllocatedError("TArray<T>::Product()  - Not Allocated Array ! ");
  T ret=(T)1;
  const T * pe;
  sa_size_t j,k;
  if (AvgStep() > 0)   {  // regularly spaced elements
...
/*! \brief Returns the sum of all array elements squared (Sum_k((*this)(k)*(*this)(k)).
  \warning: should not be called for array with complex data (will throw MathExc ) */
template <class T>
T TArray<T>::SumSq() const
{
  if (NbDimensions() < 1) 
    throw NotAllocatedError("TArray<T>::SumSq()  - Not Allocated Array ! ");
  T ret=0;
  const T * pe;
  sa_size_t j,k;
  if (AvgStep() > 0)   {  // regularly spaced elements
...
complex<T>  _ComputeComplexNorm_Private_(TArray< complex<T> > const & ca) 
{
  if (ca.NbDimensions() < 1) 
    throw NotAllocatedError("TArray< complex<T> >::Norm2()  - Not Allocated Array ! ");
...
{
  if (NbDimensions() < 1) 
    throw NotAllocatedError("TArray<T>::MinMax()  - Not Allocated Array ! ");
  const T * pe;
  sa_size_t j,k;
  int_4 ka = MaxSizeKA();
  sa_size_t step = Step(ka);
...
ostream& TArray<T>::Print(ostream& os, sa_size_t maxprt, bool si, bool ascd) const
{
  ...
  if (si) {
    bool fgdu;
    //DBG    cout << " *DBG* TArray<T>::Print() - Appel a GetUnits() ... " << endl;
    Units un = GetUnits(fgdu);
    //DBG    cout << " *DBG* TArray<T>::Print() - APRES GetUnits() fgdu= " << fgdu << endl;  
    //DBG    cout << " *DBG* TArray<T>::Print() - APRES GetUnits() un: "; un.Print(cout,2) << endl;    
