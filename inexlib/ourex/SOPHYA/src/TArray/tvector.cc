...
//! Return a subvector defined by the range of elements \b relt
template <class T>
...
/*!
  \brief return a copy of the sub-vector defined by the range of elements \b relt , as a packed vector

  The vector type (row/column vector) and the memory mapping can be changed through the arguments \b lcv and \b mm
*/
template <class T>
TVector<T> TVector<T>::PackAndCopySubMatrix(Range relt, short lcv, short mm) const 
{
  if ((lcv!=BaseArray::ColumnVector)&&(lcv!=BaseArray::RowVector))  lcv=GetVectorType();
  if ((mm!=BaseArray::CMemoryMapping)&&(mm!=BaseArray::FortranMemoryMapping))  mm=GetMemoryMapping();
  TVector<T> sv=SubVector(relt);
  TVector<T> rsv(sv.Size(),lcv,mm);
  if (rsv.GetVectorType() != sv.GetVectorType())  sv.TransposeSelf();
  rsv=sv;
  return rsv;
}
