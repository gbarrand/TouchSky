...
/*!
   ...
   Check the following web sites for definition and conventions:
...
bool Units::isDimensionLess(Units const& un) const
{
  if (!mLength_.isZero() || !mMass_.isZero() || !mTime_.isZero() ||
      !mCurr_.isZero() || !mTemp_.isZero() || !mSubst_.isZero() ||  !mLumInt_.isZero())  return false;
  return true;
}
