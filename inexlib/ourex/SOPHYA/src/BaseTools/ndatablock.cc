...
// LAL (Orsay) / IN2P3-CNRS  Irfu/SPP (Saclay) / CEA
#include "sopnamsp.h"
#include "machdefs.h"
#include <stdio.h>
#include <stdlib.h>
...
/*!
  ...
  \brief Container of data with reference sharing

  This class handles memory management for arrays of numerical types. This class implements 
  reference counting and manages transparently memory allocation/deallocation. The operations 
  follow general SOPHYA rules: The default copy constructor shares the data, incrementing the 
  reference count. The memory is freed by the destructor when the reference count reaches zero. 

  Methods are provided to clone the object, making a deep copy by allocating a new memory area.
  The class provides also basic arithmetic operations (addition,subtraction,multiplication,division)
  between two arrays (same data type) and with numerical constants. 

  The NDataBlock<T> objects can be instanciated with the following data types T:
  \verbatim
  ---- Unsigned integers :
  T = uint_1 : unsigned one   byte  integer  (8 bits)
  T = uint_2 : unsigned two   bytes integer  (16 bits)
  T = uint_4 : unsigned four  bytes integer  (32 bits)
  T = uint_8 : unsigned eight bytes integer  (64 bits)
  ---- Signed integers :
  T = int_1 : signed one   byte  integer  (8 bits) or char type
  T = int_2 : signed two   bytes integer  (16 bits)
  T = int_4 : signed four  bytes integer  (32 bits)
  T = int_8 : signed eight bytes integer  (64 bits)
  ---- Floating point numbers :
  T = r_4 : single precision float (32 bits) or float type
  T = r_8 : double precision float (64 bits) or double type
  T = r_16 (*) : quadruple precision float (128 bits) or long double type
  ---- Complex numbers :
  T = complex<r_4> : single precision complex (2x32 bits) 
  T = complex<r_8> : double precision float (2x64 bits) 
  T = complex<r_16> (*) : quadruple precision float (2x128 bits)

  -- Note : availability of r_16 and complex<r_16> depends on SOPHYA configuration 
  and compilations flags.
  \endverbatim

  \warning Note that the arithmetic operations method exists in two versions, methods performing 
  in place operations, modifying the source array (AddCst/SubCst/MulCst/DivCst) and 
  (AddElt/SubElt/MulElt/DivElt), as well as methods returning a new array containing the operation 
...
/* $CHECK$ - Reza, 22/08/2015 - le compilo dit que ca ne fait pas de sens de faire ce test, &a est non null 
...
/* $CHECK$ - Reza, 22/08/2015 - le compilo dit que ca ne fait pas de sens de faire le test if(&a!=NULL)  - toujours vrai 
   test supprime avant le cout << */
  cout<<" a.(mSz="<<a.mSz<<" mSRef="<<a.mSRef<<" IsTemp="<<a.IsTemp()<<")";
...
/*! \brief Return the sum of \b n data elements beginning at element \b i1
  specify n=0 to do the operation on all elements from \b i1 to the end */
template <class T>
...
/*! \brief Return the product of \b n data elements beginning at element \b i1
  specify n=0 to do the operation on all elements from \b i1 to the end */
template <class T>
...
/*! \brief Return the sum of the square  \b n data elements beginning at element \b i1   ( Sum[d^2] )
  specify n=0 to perform the operation on all elements from \b i1 to the end */
template <class T>
T NDataBlock<T>::SumSq(size_t i1,size_t n) const
{
if(i1>=mSz) return 0;
size_t nmax=mSz-i1;
if(n==0) n = nmax;
...
T const *p=Begin()+i1, *pe=p+n;
T x,val = 0;
while (p<pe) { x=*p++; val += x*x; }
return val;
}

template <class T>
void NDataBlock<T>::MinMax(T& min, T& max, size_t i1,size_t n) const
{
min=max=T(0);
if(i1>=mSz) return;
size_t nmax=mSz-i1;
...
void NDataBlock< complex<r_4> >::MinMax(complex<r_4>& min, complex<r_4>& max, size_t i1,size_t n) const
{
  throw MathExc("NDataBlock< complex<r_4> >::MinMax(...) - No order in complex");
}
DECL_TEMP_SPEC  /* equivalent a template <> , pour SGI-CC en particulier */
void NDataBlock< complex<r_8> >::MinMax(complex<r_8>& min, complex<r_8>& max, size_t i1,size_t n) const
{
  throw MathExc("NDataBlock< complex<r_8> >::MinMax(...) - No order in complex");
}
#ifdef SO_LDBLE128
DECL_TEMP_SPEC  /* equivalent a template <> , pour SGI-CC en particulier */
void NDataBlock< complex<r_16> >::MinMax(complex<r_16>& min, complex<r_16>& max, size_t i1,size_t n) const
{
  throw MathExc("NDataBlock< complex<r_16> >::MinMax(...) - No order in complex");
}
#endif
...
if(a.mSz==0)
  throw(NotAllocatedError("NDataBlock::operator=A null size \n"));
...
if(mSz==0) throw(NotAllocatedError("NDataBlock::operator=v null size\n"));
...
// Add a constant : ND -= b
template <class T>
NDataBlock<T>& NDataBlock<T>::AddCst(T b)
{
if(mSz==0) throw(NotAllocatedError("NDataBlock::AddCst() null size\n"));
...
/*! 
   \param fginv : if false (default), perform the operation in normal order ND.elem[k] -= b
   if true, the operands order is exchanged: ND.elem[k] = b-ND.elem[k] 
*/
template <class T>
NDataBlock<T>& NDataBlock<T>::SubCst(T b, bool fginv)
{
if(mSz==0) throw(NotAllocatedError("NDataBlock::SubCst() null size\n"));
T *p=Begin(), *pe=End(); 
if (fginv) while (p<pe) { *p = b - (*p);  p++; }
else while (p<pe) *p++ -= b;
return *this;
}
...
template <class T>
NDataBlock<T>& NDataBlock<T>::MulCst(T b)
{
if(mSz==0) throw(NotAllocatedError("NDataBlock::MulCst() null size\n"));
T *p=Begin(), *pe=End(); while (p<pe) *p++ *= b;
return *this;
}
...
/*! 
   ...
   \warning : there is no protection for divide by zero if the operation order is reversed ( b / ND.elem[k] )
*/
template <class T>
NDataBlock<T>& NDataBlock<T>::DivCst(T b,bool fginv)
{
if(mSz==0) throw(NotAllocatedError("NDataBlock::DivCst() null size\n"));
T *p=Begin(), *pe=End(); 
if (fginv) {
  if(b==(T) 0) throw(ParmError("NDataBlock::DivCst() divide by zero\n"));
  while (p<pe) { *p = b / (*p);  p++; }
}
else while (p<pe) *p++ /= b;
return *this;
...
template <class T>
NDataBlock<T>& NDataBlock<T>::AddElt(const NDataBlock<T>& a)
{
if(mSz==0 || mSz!=a.mSz)
  throw(SzMismatchError("NDataBlock::AddElt() size mismatch/null"));
T *p=Begin(), *pe=End();
T const * pa=a.Begin();
while (p<pe) *p++ += *pa++;
return *this;
...
/*! 
   \param fginv : if false (default), perform the operation in normal order ND.elem[k] -= a.elem[k]
   if true, the operands order is exchanged: ND.elem[k] = a.elem[k]-ND.elem[k] 
*/
template <class T>
NDataBlock<T>& NDataBlock<T>::SubElt(const NDataBlock<T>& a, bool fginv)
{
if(mSz==0 || mSz!=a.mSz)
  throw(SzMismatchError("NDataBlock::SubElt() size mismatch/null"));
T *p=Begin(), *pe=End();
T const *pa=a.Begin();
if (fginv) while (p<pe) { *p = *pa - *p;  p++ ; pa++; }
else while (p<pe) *p++ -= *pa++;
return *this;
}

// Multiply by a NDataBlock : ND *= NDa
template <class T>
NDataBlock<T>& NDataBlock<T>::MulElt(const NDataBlock<T>& a)
{
if(mSz==0 || mSz!=a.mSz)
  throw(SzMismatchError("NDataBlock::MulElt() size mismatch/null"));
T *p=Begin(), *pe=End();
T const *pa=a.Begin();
while (p<pe) *p++ *= *pa++;
return *this;
...
/*! 
   ...
   \warning : there is no protection for divide by zero
*/

template <class T>
NDataBlock<T>& NDataBlock<T>::DivElt(const NDataBlock<T>& a, bool fginv)
// Attention, aucune protection si un element de "a" est nul.
{
if(mSz==0 || mSz!=a.mSz)
  throw(SzMismatchError("NDataBlock::DivElt() size mismatch/null"));
T *p=Begin(), *pe=End();
T const *pa=a.Begin();
// ATTENTION !  Division par zero non protegee
if (fginv) while (p<pe) { *p = *pa / *p;  p++ ; pa++; }
else while (p<pe) *p++ /= *pa++; 
return *this;
}
