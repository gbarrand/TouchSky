...
/*!
   ...
   string, boolean, integer (\b int_8), float (\b r_8), complex and 
   date/time (TimeStamp). Date/time values are kept internally 
   as double precision values corresponding to number of days since 1 january 1901 
   and are thus less precise than TimeStamp objects. 
...
MuTyV::operator string() const 
{
  if (typ == MTVString)  return(*strv);
  ...
  else if (typ == MTVBoolean) { return (iv?"TRUE":"FALSE"); }
  else {
    char buff[96];
    if (typ == MTVCharacter) { 
      char cc = (char)(iv);
    ...
    else if (typ == MTVInteger)  sprintf(buff,"%ld", (long)iv);
...
string & MuTyV::Convert(string & x) const 
{
  ...
  else if (typ == MTVBoolean) { x = (iv?"TRUE":"FALSE"); }
  else {
    char buff[96];
    if (typ == MTVCharacter) { 
      char cc = (char)(iv);
