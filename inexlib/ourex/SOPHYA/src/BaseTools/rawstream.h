//-----------------------------------------------------------
// Class  RawInOutStream , RawInFileStream , RawOutFileStream
//        RawInOutFileStream 
...
public:
  ...
  virtual RawInOutStream& seekg(int_8, int sd = std::ios::beg);
  inline  RawInOutStream& seekg_beg() { return seekg((int_8)0, std::ios::beg); }
  inline  RawInOutStream& seekg_end() { return seekg((int_8)0, std::ios::end); }
  ...
  virtual RawInOutStream& seekp(int_8, int sd = std::ios::beg);
  inline  RawInOutStream& seekp_beg() { return seekp((int_8)0, std::ios::beg); }
  inline  RawInOutStream& seekp_end() { return seekp((int_8)0, std::ios::end); }
...
public:
                      ...
                      RawInFileStream(std::string const & path);
...
public:
                      ...
                      RawOutFileStream(std::string const & path);
  virtual             ~RawOutFileStream();

  virtual bool        isSeekable() const ;
  // Interface pour ecriture et positionnement index/position ecriture 
  virtual int_8       tellp() const ;
  virtual RawInOutStream& seekp(int_8 pos, int sd = std::ios::beg);
  virtual RawInOutStream& write(const char* s, uint_8 n);
  virtual void            flush();
protected:
  FILE * fip;
...
public:
                      ...
                      RawInOutFileStream(std::string const & path);
