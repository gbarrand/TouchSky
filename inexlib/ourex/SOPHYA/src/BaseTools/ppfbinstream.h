//-----------------------------------------------------------
// Class  PPFBinaryIOStream , PPFBinaryInputStream , PPFBinaryOutputStream
// PPF (Portable Persistence Format) Input/Output streams
// SOPHYA class library - (C)  UPS+LAL IN2P3/CNRS  , CEA-Irfu
// E. Aubourg     CEA DAPNIA/SPP  1999  - Original version 
// R. Ansari      LAL IN2P3/CNRS  2000-2003 - major update 
//-----------------------------------------------------------
...
public:
  ...
  std::string GetTagName(int itag);  // 0..NbTags-1
  std::vector< std::string > const &  GetNameTags();  

protected:
  // La liste des NameTag ds le flot  
  std::map<std::string, int_8> tags;
...
public:
  ...
  PPFBinaryInputStream(std::string const& flnm, bool scan=true);
  virtual  ~PPFBinaryInputStream();

  //! return the file name 
  inline std::string FileName() { return s->getFileName(); }   // Retourne le nom de fichier
  // ! return the RawInOutStream stream 
  inline RawInOutStream& getStream()  { return (*s); }   
  
  // Gestion des tags
  bool   GotoPositionTag(int_8 pos);
  bool   GotoNameTag(std::string const& name);
  ...
  void   GetZ4   (std::complex<r_4> &);
  void   GetZ4s  (std::complex<r_4> *, size_t);
  void   GetZ8   (std::complex<r_8> &);
  void   GetZ8s  (std::complex<r_8> *, size_t);
#ifdef SO_LDBLE128
  void   GetR16  (r_16&);
  void   GetR16s (r_16*, size_t);
  void   GetZ16  (std::complex<r_16> &);
  void   GetZ16s (std::complex<r_16> *, size_t);
#endif
  ...
  inline void   Get(std::complex<r_4> & x) {GetZ4(x);}
  inline void   Get(std::complex<r_8> & x) {GetZ8(x);}
  ...
  inline void   Get(std::string& x) {GetStr(x);}
  inline void   Get(std::complex<r_4> * x, size_t n) { GetZ4s(x, n); }
  inline void   Get(std::complex<r_8> * x, size_t n) { GetZ8s(x, n); }
#ifdef SO_LDBLE128
  inline void   Get(r_16&   x) {GetR16(x);}
  inline void   Get(r_16*   x, size_t n) {GetR16s(x,n);}
  inline void   Get(std::complex<r_16> & x) {GetZ16(x);}
  inline void   Get(std::complex<r_16> * x, size_t n) { GetZ16s(x, n); }
#endif  
  // Reading a list (table) of position tags
  void    GetPosTagTable(int_8*, size_t);
  void    GetPosTagTable(std::vector<int_8>&);
...
public:
  ...
  PPFBinaryOutputStream(std::string const& flnm, int endianness = PPS_NATIVE);
  ...
  void  WriteNameTag(std::string const& name);
  inline void WriteTag(std::string const& name) { WriteNameTag(name); }
  ...
  void PutZ4   (std::complex<r_4>);
  void PutZ4s  (std::complex<r_4> const*, size_t);
  void PutZ8   (std::complex<r_8>);
  void PutZ8s  (std::complex<r_8> const*, size_t);
#ifdef SO_LDBLE128
  void PutR16  (r_16);
  void PutR16s (r_16 const*, size_t);
  void PutZ16  (std::complex<r_16>);
  void PutZ16s (std::complex<r_16> const*, size_t);
#endif  
  
  void   Put(char   c) {PutByte(c);}
  void   Put(r_4    x) {PutR4(x);}
  void   Put(r_8    x) {PutR8(x);}
  void   Put(std::complex<r_4>   x) {PutZ4(x);}
  void   Put(std::complex<r_8>   x) {PutZ8(x);}
  ...
  void   Put(std::complex<r_4> const*    x, size_t n) {PutZ4s(x,n);}
  void   Put(std::complex<r_8> const*    x, size_t n) {PutZ8s(x,n);}
#ifdef SO_LDBLE128
  void   Put(r_16   x) {PutR16(x);}
  void   Put(r_16 const*   x, size_t n) {PutR16s(x,n);}
  void   Put(std::complex<r_16>  x) {PutZ16(x);}
  void   Put(std::complex<r_16> const*   x, size_t n) {PutZ16s(x,n);}
#endif
  ...
  void   Put(std::string const& s) {PutStr(s);}
  
  // Writing a list of position tag table
  void   PutPosTagTable(int_8 const *, size_t);
  void   PutPosTagTable(std::vector<int_8> const&);
