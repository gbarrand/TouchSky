...
//! Persistent (delegate or mixin) base class 
  ...
  public:
    ...
    void               Write(POutPersist&, std::string const& tag) const; 
    void               ReadAtTag(PInPersist& s, std::string const& tag);
...
// Handles (statically) the registration of classes.
  ...
  public:
    ...
    static void                  RegisterPPHandlerClass(uint_8 classId, std::string ppclass_name, ClassCreatorFunc f);
    static void                  RegisterDataObjClass(uint_8 classId, std::string class_name);
    ...
    static std::string           getPPClassName(uint_8 classId);
    static uint_8                getPPClassId(std::string const & typ_name);
    static uint_8                getPPClassId(PPersist const & ppo);
    static bool                  checkPPClassId(PPersist const & ppo);

    static std::string           getDataObjClassName(uint_8 classId);
    static uint_8                getDataObjClassId(std::string const & typ_name);
    static uint_8                getDataObjClassId(AnyDataObj const & o);

    static uint_8 Hash(std::string const& typname);
  ...
  private:
    ...
    typedef std::map<uint_8, ClassCreatorFunc, std::less<uint_8> > ClassList;  
    ...
    static std::map<std::string, uint_8> * ppclassNameList;  // PPersist classId = f(PPersistClassName)
    static std::map<std::string, uint_8> * dobjclassNameList;  // PPersist classId = f(DataObjClassName)
...
//! Input stream for PPersit objects.
  ...
  public:
    ...
    PInPersist(std::string const& flnm, bool scan=true);
    virtual ~PInPersist();

    // A faire - Reza Dec 2003
    std::string     GetTagClassName(int itag);
    //   Object Reading 
    PPersist*  ReadObject();
    void       GetObject(AnyDataObj & o);
    void       GetObject(AnyDataObj & o, std::string  tagname);
  ...
  protected:
    ...
    typedef std::map<uint_8, PPersist * > ObjList;
    ObjList objList;
...
//! Output stream for PPersit objects.
  ...
  public:
    ...
    POutPersist(std::string const& flnm, int endianness = PPS_NATIVE);
    virtual ~POutPersist();
    ...
    void PutObject(AnyDataObj & o, std::string tagname);
  ...
  protected:
    ...
    typedef std::map<uint_8, objreftag, std::less<uint_8> >  ObjList;
    ObjList objList;
...
// pour les types de base r_4 r_8 int_4 int_8 ...
  ...
  RAWPERSISTIO(int_1,I1);
  RAWPERSISTIO(uint_1,U1);
  RAWPERSISTIO(char,Byte);
  RAWPERSISTIO(r_4,R4);
  RAWPERSISTIO(r_8,R8);
  RAWPERSISTIO(std::complex<r_4>,Z4);
  RAWPERSISTIO(std::complex<r_8>,Z8);
  RAWPERSISTIO(std::string,Str);
#ifdef SO_LDBLE128
  RAWPERSISTIO(r_16,R16);
  RAWPERSISTIO(std::complex<r_16>,Z16);
#endif
...
//! template class for handling the PPersist registration mechanism.
  template <class T>
    ...
    public:
      ...
      static void Register(std::string id) { PIOPersist::RegisterPPHandlerClass(Hash(id), typeid(T).name(), Create); }
      static uint_8 Hash(std::string id) {
