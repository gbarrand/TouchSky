...
for(it = dvl.mvlist.begin(); it != dvl.mvlist.end(); it++)
  ...
  switch ((*it).second.elval.Type()) 
    ...
    case MuTyV::MTVBoolean :
      bool bfg;    (*it).second.elval.Convert(bfg);
      SetB((*it).first, bfg);
      break;
    case MuTyV::MTVCharacter :
      char cc;    (*it).second.elval.Convert(cc);
      SetC((*it).first, cc);
      break;
...
/* --Methode-- */
/*! Returns the value corresponding to name \b key, converted to boolean
    Default value \b def is returned if name \b key not found */
bool        DVList::GetB(string const& key, bool def) const
{
ValList::const_iterator it = mvlist.find(key);
if (it == mvlist.end())  return(def);
return( (*it).second.elval.Convert(def) );
}

/*! Returns the value corresponding to name \b key, converted to character
    Default value \b def is returned if name \b key not found */
char        DVList::GetC(string const& key, char def) const
{
ValList::const_iterator it = mvlist.find(key);
if (it == mvlist.end())  return(def);
return( (*it).second.elval.Convert(def) );
...
if (it == mvlist.end())  return(def);
string rs;
return( (*it).second.elval.Convert(rs) );
}

/* --Methode-- */
/*! Returns the value corresponding to name \b key, converted to TimeStamp
    Default value \b def is returned if name \b key not found */
TimeStamp   DVList::GetT(string const& key, TimeStamp def) const
{
ValList::const_iterator it = mvlist.find(key);
if (it == mvlist.end())  return(def);
return( (*it).second.elval.Convert(def) );
...
/* --Methode-- */
/*! Appends or sets the bool value \b val in the list with name \b key */
void        DVList::SetB(string const& key, bool val)
{
MuTyV div(val);
Get(key) = div;
}
...
/* --Methode-- */
/*! Appends or sets the char value \b val in the list with name \b key */
void        DVList::SetC(string const& key, char val)
{
MuTyV div(val);
Get(key) = div;
}
...
for(it = mvlist.begin(); it != mvlist.end(); it++)  {
  switch ((*it).second.elval.Type()) 
    {
    case MuTyV::MTVBoolean :
      (*it).second.elval.Convert(fgb);
      sprintf(buff, "%s = %s (T=Boolean) /  %s\n", (*it).first.substr(0,64).c_str(), 
	      (fgb?"TRUE":"FALSE"), (*it).second.elcomm.substr(0,128).c_str());
      break;
    case MuTyV::MTVCharacter :
      (*it).second.elval.Convert(cc);
      sprintf(buff, "%s = %c (T=Character) /  %s\n", (*it).first.substr(0,64).c_str(), 
	      cc, (*it).second.elcomm.substr(0,128).c_str());
      break;
    case MuTyV::MTVInteger :
...
uint_4 itab[4];
itab[0] = 4;  // Version number = 4 , depuis Mars 2018 , correction et modification format Ecriture TimeStamp 
itab[1] = dobj->Size(); 
itab[2] = lc; 
itab[3] = 0;
s.Put(itab, 4);
...
for(it = dobj->Begin(); it != dobj->End(); it++)  {
  switch ((*it).second.elval.Type()) {
    case MuTyV::MTVBoolean :
      (*it).second.elval.Convert(fgb);
      sprintf(buf,"B %s %s\n", (*it).first.substr(0,64).c_str(), (fgb?"true":"false") );
      sfw = buf;  s.PutStr(sfw);
      break;
    case MuTyV::MTVCharacter :
      (*it).second.elval.Convert(cc);
      sprintf(buf,"C %s %c\n", (*it).first.substr(0,64).c_str(), cc);
      sfw = buf;  s.PutStr(sfw);
      break;
    case MuTyV::MTVInteger :
    ...
    case MuTyV::MTVTimeStamp : 
      (*it).second.elval.Convert(ts);
      sprintf(buf,"T %s %s\n", (*it).first.substr(0,64).c_str(), ts.ToString().c_str());
      sfw = buf;  s.PutStr(sfw);
      break;
    default :
      break;
...
while(ok) {
  ...
  switch (buf[0]) {
    case 'B' :
      fgb=false;
      if (strncmp(buf+j+1,"true",4)==0)  fgb=true;
      key = buf+2;
      dobj->SetB(key, fgb);
      break;
    case 'C' :
    ...
    case  'S' :
      ...
      break;
    case 'T' :
      if (itab[0] >= 4) {  //  V >= 4 , on ecrit un TimeStamp au format packed avec un ToString 
        TimeStamp ts(buf+j+1);
        key = buf+2;
        dobj->SetT(key, ts);
      }
      else {  // V <= 3 TimeStamp ecrit apres conversion en double 
	dv = atof(buf+j+1);
	key = buf+2;
