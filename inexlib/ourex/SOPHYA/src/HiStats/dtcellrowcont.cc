...
/*!
   ...
   of the table. It provides a more efficient way of accessing the data
   compared to the more easy to use DataTableRow class, minimizing converion 
   and memory to memory data copies.

   \warning  the use of the equal (=) operator is not permitted on DataTableRowPtr objects. 
*/
...
/*!
  prints the DataTableRowPtr object content on the stream os.
  \param csep : separator string inserted between columns 
  \param vsep : separator string between vector column elements
*/
ostream&   DataTableRowPtr::Print(ostream& os, const char* csep, const char* vsep) const
{
  ...
  for(size_t k=0; k<Size(); k++) {
	...
	if (j!=cellsizes_[k]-1)  os << vsep; 
      ...
      break;
    case DTC_CharacterType:
      for(size_t j=0; j<cellsizes_[k]; j++) {
 	os << *(cellptr_[k].Ptr().cp+j);
	if (j!=cellsizes_[k]-1)  os << vsep; 
      }
      break;
	...
	if (j!=cellsizes_[k]-1)  os << vsep; 
      ...
      break;
    case DTC_FlagVec16Type:
      for(size_t j=0; j<cellsizes_[k]; j++) {
 	os << *(cellptr_[k].Ptr().b16+j);
	if (j!=cellsizes_[k]-1)  os << vsep; 
      }
     break;
    case DTC_FlagVec64Type:
      for(size_t j=0; j<cellsizes_[k]; j++) {
 	os << *(cellptr_[k].Ptr().b64+j);
	if (j!=cellsizes_[k]-1)  os << vsep; 
      }
     break;
...
size_t  DataTableRowPtr::getDataSize(DataTableCellType typ)
{
  ...
  case DTC_FMLStr64Type:
    ...
    break;
  case DTC_FlagVec16Type:
    return sizeof(FlagVector16);
    break;
  case DTC_FlagVec64Type:
    return sizeof(FlagVector64);
    break;
  default:
    return 0;
    break;
