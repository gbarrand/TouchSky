...
/*! 
  \brief return the data for the specified row from the table-like object  
...
/*!
  \brief return data for cell in column \b k in row n from the table-like object 
...
/* --Methode-- */
//! Returns the column index given the column name - generates an exception if non existing column name is specified 
size_t NTupleInterface::ColumnIndex(string const & nom)  const
{
  throw NotFoundExc("NTupleInterface::ColumnIndex() - unknown column name");
}

/* --Methode-- */
//! Returns the column index given the column name - return -1 if unknown column name is specified 
int NTupleInterface::ColumnIndexNIU(string const & nom)  const 
{
  return -1;
}
...
/* --Methode-- */
/*!
  \brief Return true if the class implements the methods that return table content representation as a string
  
  Default implementation return false. Derived classes which implement the  LineHeaderToString() \b AND 
  LineToString() methods should also reimplement this method and set the return value to true.
*/
bool NTupleInterface::ImplementsContentStringRepresentation() const
{
  return false;
}
