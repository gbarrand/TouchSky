...
// C. Magneville      (C) DAPNIA/SPP  CEA     1996
//-----------------------------------------------------------
...
public:
  ...
  inline void  PrintStatus() const { Show(std::cout); }
...
protected:
  ...
  std::list<bande_slice>  mLBandx; //!< liste des bandes selon X
  std::list<bande_slice>  mLBandy; //!< liste des bandes selon Y
  
  std::list<bande_slice>  mLSlix; //!< liste des slices selon X
  std::list<bande_slice>  mLSliy; //!< liste des slices selon Y

};

/*! Prints histogram information on stream \b s (h.Show(s)) */
inline std::ostream& operator << (std::ostream& s, Histo2D const & h)
  {  h.Show(s);  return(s);  }
