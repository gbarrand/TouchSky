...
// R. Ansari          (C) UPS+LAL IN2P3/CNRS  2005
//-----------------------------------------------------------
...
SwPPFDataTable::SwPPFDataTable(POutPersist & os, size_t segsz)
  : BaseDataTable(segsz) ,
    ...
    mCSwapper(os), mS16Swapper(os), mS64Swapper(os),
    mB16Swapper(os), mB64Swapper(os)
{
}
...
SwPPFDataTable::SwPPFDataTable(PPFDataSwapper<int_4>& swp, size_t segsz)
  : BaseDataTable(segsz) ,
    ...
    mCSwapper(swp.OutStream(), swp.InStream()), 
    mS16Swapper(swp.OutStream(), swp.InStream()), mS64Swapper(swp.OutStream(), swp.InStream()),
    mB16Swapper(swp.OutStream(), swp.InStream()), mB64Swapper(swp.OutStream(), swp.InStream())
{
}

//! Protected constructor for creation from a swap stream 
SwPPFDataTable::SwPPFDataTable(PInPersist* isp, size_t segsz)
  : BaseDataTable(segsz) ,
    mSwap_(isp,true), 
    mISwapper(*isp), mLSwapper(*isp), 
    mFSwapper(*isp), mDSwapper(*isp), 
    mYSwapper(*isp), mZSwapper(*isp), 
    mSSwapper(*isp), mTSwapper(*isp), mCSwapper(*isp), 
    mS16Swapper(*isp), mS64Swapper(*isp),
    mB16Swapper(*isp), mB64Swapper(*isp)
{
}
...
void SwPPFDataTable::Share(SwPPFDataTable const & a)
{
  ...
  mCSwapper = a.mCSwapper;
  mS16Swapper = a.mS16Swapper;
  mS64Swapper = a.mS64Swapper;
  mB16Swapper = a.mB16Swapper;
  mB64Swapper = a.mB64Swapper;
  ...
  for (size_t kk=0; kk<mNames.size(); kk++) {
    ...
    case FMLStr64Field :
      ...
      break;
    case FlagVec16Field :
      mB16Cols[sk] = a.mB16Cols[ska]; 
      break;
    case FlagVec64Field :
      mB64Cols[sk] = a.mB64Cols[ska]; 
      break;
    default:
...
void SwPPFDataTable::SwapOutAll() const
{
  ...
  for (size_t kk=0; kk<mNames.size(); kk++) {
    ...
    case FMLStr64Field :
      ...
      break;
    case FlagVec16Field :
      mB16Cols[sk].SwapOutBuffer();
      break;
    case FlagVec64Field :
      mB64Cols[sk].SwapOutBuffer();
      break;
    default:
      throw ForbiddenError("SwPPFDataTable::Share() : unknown column type ");
    break;
  ...
  size_t SwPPFDataTable::AddColumn(FieldType ft, string const & cnom, size_t vsz, bool fgfcnet)
{
  ...
  */
  ...
  case CharacterField :
    ser = mCCols.size();
    mCCols.push_back(SwPPFSegDataBlock<char>(mCSwapper, segsz, 0, vsz));
    mCColIdx.push_back(idx);
    mCColsP.push_back(NULL);
    for(size_t kk=0; kk<mCCols.size(); kk++)
      mCColsP[kk] = &(mCCols[kk]);
    break;
  case FMLStr16Field :
    ser = mS16Cols.size();
  ...
  case FlagVec16Field :
    ser = mB16Cols.size();
    mB16Cols.push_back(SwPPFSegDataBlock< FlagVector16 >(mB16Swapper, segsz,0,vsz));
    mB16ColIdx.push_back(idx);
    mB16ColsP.push_back(NULL);
    for(size_t kk=0; kk<mB16Cols.size(); kk++)
      mB16ColsP[kk] = &(mB16Cols[kk]);
    break;
  case FlagVec64Field :
    ser = mB64Cols.size();
    mB64Cols.push_back(SwPPFSegDataBlock< FlagVector64 >(mB64Swapper, segsz,0,vsz));
    mB64ColIdx.push_back(idx);
    mB64ColsP.push_back(NULL);
    for(size_t kk=0; kk<mB64Cols.size(); kk++)
      mB64ColsP[kk] = &(mB64Cols[kk]);
    break;
