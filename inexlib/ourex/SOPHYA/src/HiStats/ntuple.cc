...
/*!
   ...
   single precision floating point value cell content.

   \warning  contrary to most SOPHYA objects, the copy constructor implements only a deep copy.
   \warning Thread safe fill operation can be activated using the method SetThreadSafe(). Default mode is NON thread-safe fill.
...
//--
{
  nomretour[0] = '\0';
  if ((k >= 0) && (k < mNVar))  { 
    strncpy(nomretour, mNames[k].c_str(), 255);
    return nomretour;
  }
  else throw ParmError("NTuple::NomIndex() out of range index k (<0 or >=NVar)");
  return(nomretour);
}
...
//JEC 2/02/2015 correction reference argument et position "i++"
...
/*!
  ...
  \param sep : character(s) separator that delimit the different fields (cells/columns) in each line of the file 
  if no separator specified (sep=NULL, default) the tab and space characters are used as separators. Up to three 
 ...
 */
//++
int  NTuple::FillFromASCIIFile(string const& fn, double defval, const char* sep, const char* com)
//
...
char sepc[3]={'\0','\0','\0'};  // separator characters 
char comc[3]={'\0','\0','\0'};  // comment characters 
if ((sep==NULL)||(sep[0]=='\0'))   { sepc[0]='\t';  sepc[1]=' '; }
else {
  sepc[0]=sep[0]; 
  if (sep[1]!='\0')  { 
    sepc[1]=sep[1];
    if (sep[2]!='\0')  sepc[2]=sep[2];
  }
}
if ((com==NULL)||(com[0]=='\0'))   comc[0]='#';  
else {
  comc[0]=com[0]; 
  if (com[1]!='\0')  { 
    comc[1]=com[1];
    if (com[2]!='\0')  comc[2]=com[2];
  }
}
...
int nlread=0, cntrd=0;
int nvar = NbColumns();
// On boucle sur toutes les lignes
while (fgets(lineb,4096,fip) != NULL) {
  lineb[4095] = '\0';
  j = 0; line = lineb;
  //DBG  cout << " *DBG* cnt="<<cntrd<<" Line= "<<line<<endl;
  cntrd++;
// On continue s'il s'agit d'une ligne commentaire 
  if ((line[0]==comc[0])||(line[0]==comc[1])||(line[0]==comc[2]))  continue;
  for(kk=0; kk<nvar; kk++)  xv[kk] = defval;
  for(kk=0; kk<nvar; kk++) {
//  On enleve les espaces de debut 
    j=0;    while ( (line[j] != '\0') && (line[j] == ' ') )  j++;
    line += j;
    ccp = line;
// On cherche le separateur 
    poss=-1;
    poss = posc(ccp, sepc[0]);
    if (sepc[1]!='\0')  {
      poss2=posc(ccp, sepc[1]);
      if ((poss2>=0)&&((poss2<poss)||(poss<0)))  poss=poss2;
      poss2=posc(ccp, sepc[2]);
      if ((poss2>=0)&&((poss2<poss)||(poss<0)))  poss=poss2;
    }
    if (poss >= 0)  ccp[poss] = '\0';
    if ( isdigit(ccp[0]) || (ccp[0] == '+') || (ccp[0] == '-') || (ccp[0] == '.') ) 
      xv[kk] = atof(ccp);
    if (poss < 0)  break;
    line += poss+1;   j = 0;
    }
...
delete[] xv;
fclose(fip);
cout << "NTuple::FillFromASCIIFile( "<<fn<<") "<<cntrd<<" lines from file -->"<<nlread<<" rows Filled"<<endl;
return(nlread);
}
...
/*!
  \param fn : file name
  \param sep : field separator character - default = space
  \param com : character used to mark comment lines

*/
int NTuple::WriteToASCIIFile(string const& fn, char sep, char com)
{
  FILE * fip = fopen(fn.c_str(), "w");
  if (fip == NULL) {
    cout << "NTuple::WriteToASCIIFile() Error opening file " << fn << endl;
    return(-2);
  }
  int i,j;
  fputc(com,fip);
  fputs("  ",fip);
  for(i=0; i<mNVar; i++)  fprintf(fip, "%s  ", mNames[i].c_str());
  fputc('\n',fip);
  fprintf(fip,"%c  NTuple: NCol=NVar=%d   NRows=%d \n",com, mNVar, mNEnt);
  for(int i=0; i<mNEnt; i++) {
    GetVecD(i, NULL); 
    for(j=0; j<mNVar; j++)  fprintf(fip, "%8lg %c ", (double)mVarD[j],sep);
    fputc('\n',fip);
  }
  fclose(fip);
  return mNEnt+2;
...
{
  GetMinMax(ColumnIndex(nom), min, max);
}

/* --Methode-- */
size_t NTuple::ColumnIndex(string const & nom)  const
{
  int idx = IndexNom(nom.c_str());
  if (idx < 0)  throw NotFoundExc("NTuple::ColumnIndex() unknown column name specified");
  return (size_t)idx;
}

/* --Methode-- */
int NTuple::ColumnIndexNIU(string const & nom)  const 
{
...
/* --Methode-- */
bool NTuple::ImplementsContentStringRepresentation() const 
{
  return true;
}
