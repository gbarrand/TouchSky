...
static double _CE_randgauss(double mu, double sig) { return GaussianRand(sig,mu); }
static double _CE_gauss0(double x) { return exp(-0.5*x*x); }
static double _CE_gaussian(double x, double mu, double sig) { x -= mu; return exp(-x*x/(2.*sig*sig)); }
...
#else  
  else if (func == "erf") _f1 = erf;
  else if (func == "erfc") _f1 = erfc;
#endif
  else if (func == "j0") _f1 = j0;
  else if (func == "j1") _f1 = j1;
  else if (func == "gauss0") _f1 = _CE_gauss0;
  else if (func == "gaussian") _f3 = _CE_gaussian;
// Les fonctions aleatoires declaree plus haut 
  else if (func == "rand01") _f0 = _CE_rand01;	
  else if (func == "randpm1") _f0 = _CE_randpm1;	
  else if (func == "randgauss") _f2 = _CE_randgauss;	
...
/*!
  ...
  The following functions from the standard math library are available: 
  
  - sqrt fabs floor hypot
  - exp log log10 pow 
  - sin cos tan asin acos atan atan2  (trigonometric functions)
  - sinh cosh tanh  (hyperbolic sin, cos, tan)
  - erf erfc (Error function (Gaussian integral) and complementary error function) 
  - j0 j1  (Bessel functions of the first kind, 0,1st order) 

 
  Few additional functions as well as random generators are also available:

  - gaussian(x, mu, x) : Unit amplitude gaussian function exp(-(x-mu)^2/(2 sig^2)
  - gauss0(x) : Unit amplitude gaussian function with mu=0, sigma=1. 
  - rand01() : Flat random number generator in the range [0 1]
  - randpm1() : Flat random number generator in the range [-1 1]
  - randgauss(sigma, mean) : Gaussian random number generator
...
//---------------------------------------------------------------------------
//---- Fonctions interpretees 
//---------------------------------------------------------------------------

class P_CE_VarList_4_Func : public CE_VarListInterface {
public:
  explicit P_CE_VarList_4_Func(const char * xname="x", const char * yname="y", const char * zname="z"):
    xname_(xname), yname_(yname), zname_(zname)
  {
    mesx[0]=mesx[1]=mesx[2]=0.;
  }
  virtual ~P_CE_VarList_4_Func() { }
 
  virtual double* GetVarPointer(std::string const& name)
  {
    if (name==xname_)  return mesx;
    else if (name==yname_)  return mesx+1;
    else if (name==zname_)  return mesx+2;
    else   return NULL;
  }
  inline void setX(double x) { mesx[0]=x; }
  inline void setY(double y) { mesx[1]=y; }
  inline void setZ(double z) { mesx[2]=z; }
  inline void setXY(double x, double y) { mesx[0]=x; mesx[1]=y; }
  inline void setXYZ(double x, double y, double z) { mesx[0]=x; mesx[1]=y; mesx[2]=z;}

  std::string xname_, yname_, zname_;
  double mesx[5];
};

//-----------------------------------------------------------------------------
/* --Methode-- */
CExpFunc1D::CExpFunc1D(const char * sex, const char* vnamex)
{
  CreateFrom(sex, vnamex);
}

/* --Methode-- */
CExpFunc1D::CExpFunc1D(string const & sex, string const & vnamex)
{
  CreateFrom(sex.c_str(), vnamex.c_str());
}

/* --Methode-- */
CExpFunc1D::CExpFunc1D(CExpFunc1D const & a)
{
  CreateFrom(a.sex_.c_str(), a.vnamex_.c_str());
}

/* --Methode-- */
void CExpFunc1D::CreateFrom(const char * sex, const char* vnamex)
{
  vlist_ = new P_CE_VarList_4_Func(vnamex);
  cexpv_ = new CExpressionEvaluator(sex, vlist_);
  sex_=sex;  vnamex_=vnamex; 
}

/* --Methode-- */
CExpFunc1D::~CExpFunc1D()
{
  delete cexpv_;
  delete vlist_;
}
...
/* --Methode-- */
double CExpFunc1D::operator()(double x) const
{
  vlist_->setX(x);
  return cexpv_->Evaluate();
}

//-----------------------------------------------------------------------------
/* --Methode-- */
CExpFunc2D::CExpFunc2D(const char * sex, const char* vnamex,const char* vnamey)
{
  CreateFrom(sex, vnamex, vnamey);
}

/* --Methode-- */
CExpFunc2D::CExpFunc2D(string const & sex, string const & vnamex, string const & vnamey)
{
  CreateFrom(sex.c_str(), vnamex.c_str(), vnamey.c_str());
}

/* --Methode-- */
CExpFunc2D::CExpFunc2D(CExpFunc2D const & a)
{
  CreateFrom(a.sex_.c_str(), a.vnamex_.c_str(), a.vnamey_.c_str());
}

/* --Methode-- */
void CExpFunc2D::CreateFrom(const char * sex, const char* vnamex,const char* vnamey)
{
  vlist_ = new P_CE_VarList_4_Func(vnamex, vnamey);
  cexpv_ = new CExpressionEvaluator(sex, vlist_);
  sex_=sex;  vnamex_=vnamex;  vnamey_=vnamey; 
}

/* --Methode-- */
CExpFunc2D::~CExpFunc2D()
{
  delete cexpv_;
  delete vlist_;
}
...
/* --Methode-- */
double CExpFunc2D::operator()(double x, double y) const
{
  vlist_->setXY(x,y);
  return cexpv_->Evaluate();
}
...
//-----------------------------------------------------------------------------
/* --Methode-- */
CExpFunc3D::CExpFunc3D(const char * sex, const char* vnamex,const char* vnamey, const char* vnamez)
{
  CreateFrom(sex, vnamex, vnamey, vnamez);
}

/* --Methode-- */
CExpFunc3D::CExpFunc3D(string const & sex, string const & vnamex, string const & vnamey, string const & vnamez)
{
  CreateFrom(sex.c_str(), vnamex.c_str(), vnamey.c_str(), vnamez.c_str());
}

/* --Methode-- */
CExpFunc3D::CExpFunc3D(CExpFunc3D const & a)
{
  CreateFrom(a.sex_.c_str(), a.vnamex_.c_str(), a.vnamey_.c_str(), a.vnamez_.c_str());
}

/* --Methode-- */
void CExpFunc3D::CreateFrom(const char * sex, const char* vnamex, const char* vnamey, const char* vnamez)
{
  vlist_ = new P_CE_VarList_4_Func(vnamex, vnamey, vnamez);
  cexpv_ = new CExpressionEvaluator(sex, vlist_);
  sex_=sex;  vnamex_=vnamex;  vnamey_=vnamey;   vnamez_=vnamez;  
}

/* --Methode-- */
CExpFunc3D::~CExpFunc3D()
{
  delete cexpv_;
  delete vlist_;
}
...
/* --Methode-- */
double CExpFunc3D::operator()(double x, double y, double z) const
{
  vlist_->setXYZ(x,y,z);
  return cexpv_->Evaluate();
}
