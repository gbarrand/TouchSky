...
class FitsManager {
 public:
  ...
  static int  RegisterHandler(FitsHandlerInterface * fhi, int glev=0, std::string desc="");
  ...
  static int ScanFile(std::string filename, std::ostream& os, bool fgph=false, bool fgrd=false);
  //! Scans the fits file and prints information about each HDU on \b cout
  static inline int ScanFile(std::string filename, bool fgph=false, bool fgrd=false)
  { return ScanFile(filename, std::cout, fgph, fgrd); }
