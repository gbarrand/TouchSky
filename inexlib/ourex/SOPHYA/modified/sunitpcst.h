...
public:
  ...
  explicit Units(const std::string & s);
  ...
  //! return true if the quantity is dimensionless (true for angles, counts ...)
  bool isDimensionLess(Units const& un) const;
  ...
  inline std::string const& Name() const { return mName_; }
  //! return the unit short name 
  inline std::string const& ShortName() const { return mShortName_; }
  //! Prints the object on \b cout (return the cout stream object)
  inline std::ostream& Print(int lev=2)  const  
  { return Print(std::cout,lev); }  
  //! Prints the object on stream \b os (return the os stream object)
  virtual std::ostream& Print(std::ostream& os,int lev=2)  const;
  //! return a complete representation of the object as a string
  std::string ToStringFullForm() const;
  ...
  // Handle the case of Microsoft Visual c++ which defines a macro pascal 
#ifdef _MSC_VER
#define pascal sophya_units_pascal
#endif
#if TARGET_OS_IPHONE
#define pascal sophya_units_pascal
#endif  
  inline static Units pascal()
...
protected:
  inline void SetName(std::string const& nom, std::string const& snom)
  ...
  std::string mName_, mShortName_;   // Name and short name 
};

//! operator << overloading - Prints Units in string format on \b os
  inline std::ostream& operator << (std::ostream& s, Units const & un)
...
public:
  ...
  inline Units getUnit() const  {return unit_; }
  ...
  inline void setNameDesc(std::string const& name) { name_=name; }

  // return true if the two objects are equal (same unit). 
  bool isEqual(PhysQty const& qty, bool fgstrict=true) const;

  //! Prints the object on \b cout (return the cout stream object)
  inline std::ostream& Print(int lev=0)  const  
  { return Print(std::cout, lev); }  
  //! Prints the object on stream \b os (return the os stream object)
  virtual std::ostream& Print(std::ostream& os,int lev=2)  const;
...
protected:
  ...
  std::string name_;  // Name of the quantity / constant 
};

//! operator << overloading - Prints PhysQty on \b os
inline std::ostream& operator << (std::ostream& s, PhysQty const & qty)
